...
- P0 当前任务卡现在还能生成可移植的 P0 可用性 Markdown 报告：`buildP0PlatformReadinessReportMarkdown` 会把 Golden Path 完成度、当前缺口、开放 P0 缺口队列、最近审计或 paper-only 执行证据、证据深链和 paper-only/live-blocked 边界整理为一份可复制或下载的文本报告。首页提供“复制报告”和“下载报告”动作，方便人工复核、跨会话沟通和问题定位；该报告只复用前端已展示的 P0 证据，不写后端审计账本、不创建委托、不改变执行状态，也不授权实盘交易。
