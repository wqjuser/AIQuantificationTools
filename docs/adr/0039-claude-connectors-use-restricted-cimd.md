...
- 迁移工具只把版本库冻结的 CIMD、Claude Hosted 预注册客户端、realm default scopes，以及 `basic`/`aiqt:research:read` scope 与 mapper 字段写入内网 Keycloak Admin API；旧 `aiqt-mcp` 只允许原地改名，不能与新 Client ID 并存。Audience mapper 必须精确指向 canonical `/mcp` resource，缺失、额外或漂移均失败关闭。重复执行幂等；管理员密码只通过交互提示进入进程，不写入参数、环境、日志或 Git。
