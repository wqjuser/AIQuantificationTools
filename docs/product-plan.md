...
- Stage 1 研究上下文就绪清单现在纳入“刷新证据”：页面会把最近自选缓存刷新运行与当前 market/symbol/timeframe 对齐，命中且完整、无 warning、非 demo/unknown 来源时显示就绪；没有匹配刷新、刷新失败/跳过、数据不完整或来源需复核时显示为 review gate，并提供刷新缓存动作，但不会把已有可用缓存误判为硬阻断。
