...
- `runs.py`：SQLite 研究运行审计记录，保存 run id、结构化策略配置、策略 revision、数据质量、数据快照、数据行数、指标、交易流水、权益曲线、诊断、AI 决策和完整 AI 研究报告，并可序列化为可复现的研究运行导出包，也可将导出包恢复为本地审计记录。
...
- `GET /api/research/runs/{runId}/export`：返回 `aiqt.researchRun.export` JSON 包，用于离线复现和后续执行交接；包内包含 manifest、完整 `researchRun` 详情、数据快照、artifact counts 和 paper-only execution handoff，实盘闸门默认保持关闭。
- `POST /api/research/runs/import`：导入 `aiqt.researchRun.export` JSON 包，将其中的 `researchRun` 写入本地审计库并返回完整 run detail；若导入包声称允许实盘交易则返回 `invalid_research_run_export`，导入不能绕过本地风控闸门。
