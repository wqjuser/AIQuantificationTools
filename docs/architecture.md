...
- `audit_events.py`：SQLite 通用审计事件库，保存 `schemaVersion=1` 的 `AuditEvent`，支持按 `eventType`、`runId`、全文查询、`limit/offset` 分页读取；首个落地事件类型是 `research_run_import`。
...
本地核心已经提供 `AiReviewRunStore`，并通过 `/api/research/runs/{runId}/ai-reviews` 保存和读取绑定到同一审计 run 的 AI 评审记录。POST 会先确认 research run 存在，再校验 `recordType=aiqt.aiReviewRun`、`schemaVersion=1`、`runId` 与路径一致和安全边界存在；GET 支持 `limit`、`offset`、`query`，并返回 `pagination` 元信息。Audit 工作区会用同一个后端查询结果驱动漂移摘要、保存记录历史和上一页/下一页控件，避免前端二次过滤和后端搜索语义不一致；前端 `buildAiReviewAuditTimelineItems` 还会把当前审计证据、当前页保存的 AI 评审记录和风控审批状态合并成紧凑时间线引用，并为每个引用附带 `exportAnchor`、`targetWorkspaceId`、`targetRecordId` 和 `actionLabel`，让 Audit 时间线可以看到导出包锚点、跳转回测证据、切换保存评审对照或进入执行审批。`buildAiReviewExportEvidenceIndexRows` 会继续把当前 AI 评审记录、保存记录和时间线引用整理成导出证据索引行，`filterAiReviewExportEvidenceIndexRows` 支持按分组、anchor、reference、`exportPath`、说明和 tone 本地过滤，供 Audit 工作区快速定位复现包证据位置。`buildResearchRunExportPreviewRows` 则在前端把当前工作区、当前 AI 评审记录、已保存 AI 评审记录、最新模拟执行、晋级候选和风控审批合并成复现包预览行，逐项标记 `researchRun`、`dataSnapshot`、`strategyConfig`、研究笔记、回测流水、`aiReviewRuns[]`、`paperExecutions[]`、`promotionCandidate` 和 `executionHandoff.requiredGates` 的 `ready/missing/blocked` 状态；`filterResearchRunExportPreviewRows` 支持按 artifact、anchor、`exportPath`、状态和说明本地搜索。`buildResearchRunExportBrowserRows` 会读取某个历史运行的完整导出包 manifest、integrity、artifact counts 和实际包内载荷数量，生成 `package`、`integrity`、`data`、`backtest`、`research-note`、`paper-executions`、`promotion-candidate`、`ai-reviews` 和 `execution-handoff` 检查行；`filterResearchRunExportBrowserRows` 支持按 manifest 字段、`exportPath`、状态、值和说明搜索，Audit 工作区可从 Run History 直接加载并检查单个复现包，而不必先打开原始 JSON。`buildResearchRunExportIndexRows` 会把当前 Run History 批量加载得到的多个导出包压缩成近期复现包索引行，按 `exportedAt` 倒序显示 run、标的、策略 revision、integrity、data hash、bars/trades/AI review artifact 摘要和执行交接闸门，并把 integrity 缺失、数据快照数量不一致、模拟执行/晋级候选/AI 评审数量不一致标记为 blocked；`filterResearchRunExportIndexRows` 支持按 run、标的、revision、hash、artifact、执行模式、阻断原因和 `manifest:{runId}` 搜索，用于 Audit 工作区的跨包审计筛查。`normalizeResearchRunExportPackagePayload` 会把外部文件中的原始 `aiqt.researchRun.export` 或 `{ export: ... }` 包装统一成前端可检查的复现包；文件选择只会设置 pending import 和 `inspectedExportPackage`，让 `buildResearchRunImportDiffRows` 先生成 `package-integrity` 和 `artifact-counts` 安全闸门，再比较当前工作区与包内 `researchRun`/manifest/attached artifacts，输出 run id、市场标的、周期、数据快照、策略 revision、研究笔记、模拟执行、AI 评审和实盘边界的 `same/add/change/replace/blocked` 字段级导入影响。完整性 hash 无效、manifest dataRows 与 bars 不一致、研究笔记/模拟执行/晋级候选/AI 评审数量和包内载荷不一致都会在前端预检阶段标记为 `blocked`，并复用确认按钮的 blocked gate，只有用户在无阻断的预检面板确认后，前端才调用 `/api/research/runs/import` 写入本地核心；前端导入客户端会保留本地核心返回的 `detail` 错误文本，避免把 400 导入拒绝误显示成离线 fallback。`buildResearchRunImportAuditEvent`、`buildResearchRunImportUndoAuditEvent`、`buildResearchRunImportUndoConfirmation`、`mergeResearchRunImportAuditEvents` 和 `filterResearchRunImportAuditEvents` 会把本页外部导入的预检、阻断、取消、失败、确认写入、撤销确认和撤销完成状态压缩成可搜索的导入审计流水；事件 metadata 记录 `previousRunId`、`rollbackTargetRunId`、`failureCategory`、`recoveryHint` 和后端返回的 `undoToken`，确认导入后可先在行内打开不可重复撤销提示，再调用 `/api/research/runs/import/undo` 主动撤销本次写入，撤销成功后同一个 `AuditEvent.eventId` 会 upsert 为 `stage=undone`，隐藏重复撤销动作并显示 token 已消费，也保留旧 audited run 回放作为上下文恢复兜底；前端会把它们转换为 `eventType=research_run_import` 的 `AuditEvent` 保存到 `/api/audit/events`，进入 Audit 工作区时再从后端回读最近事件。后端导入写入阶段会先全量快照目标 run、研究笔记、策略版本、模拟执行和 AI 评审记录，任一 store 写入失败都会返回 `research_run_import_write_failed`，并通过 `delete`/`restore`/`delete_by_run` 补偿恢复导入前状态或删除本次半写入记录；成功导入会写入 `ResearchRunImportUndoStore`，撤销接口会恢复导入前快照或删除新增记录，并把 token 标记为 consumed。
...
- `POST /api/research/runs/import/undo`：请求体为 `{ "undoToken": "..." }`，按成功导入时保存的快照恢复目标 run、研究笔记、策略版本、模拟执行和 AI 评审记录；若导入前不存在对应记录，则删除本次导入新增记录。撤销成功后 token 会标记为 consumed 并返回恢复后的 run detail；token 不存在返回 `research_run_import_undo_not_found`，重复使用返回 `research_run_import_undo_already_consumed`。
...
- `POST /api/audit/events`：保存一条通用审计事件，当前用于导入复现包的预检、阻断、取消、失败、确认写入和撤销完成流水；同一个 `eventId` 会 upsert，因此前端可把已确认导入事件刷新为 `undone` 状态。导入事件 metadata 会携带旧 run、可回放恢复目标、失败分类、undo token 和恢复提示，响应返回规范化后的 `event`。
- `GET /api/audit/events?eventType=research_run_import&runId=...&limit=20&offset=0&query=`：按事件类型、run id 和全文 query 读取审计事件分页，返回 `events` 与 `pagination`，供 Audit 工作区恢复最近导入审计流水和旧 run 回放入口。
