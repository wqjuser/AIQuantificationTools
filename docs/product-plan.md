...
- 阶段 5 · 实盘准备：当前唯一交付阶段；Shadow Safety Drills 已完成，当前交付服务端权威 Sandbox 准入门禁与便携审计，不连接 broker/testnet 或资金账户。
...
阶段 4 只允许维护和阻断性缺陷修复；Stage 5 当前只允许生成 `ready_for_manually_authorized_sandbox_phase` 证据决策。选择第二个 sandbox adapter、连接 broker/testnet、读取 secret、账户同步、订单提交和 live route 均须进入后续独立设计、人工授权与验收。
