...
Batch 82 已把本地复核覆盖下一步的两个落地动作也改成 action-specific：查询按钮会显示“查看 Daily Ops 覆盖查询 / View Daily Ops coverage query”或“查看个人/小团队覆盖查询 / View personal/team coverage query”，入口按钮会显示“打开 Daily Ops 复核入口 / Open Daily Ops review entry”或“打开个人/小团队复核入口 / Open personal/team review entry”。两个按钮点击后会分别写入 `Daily Ops coverage query selected`、`Personal/team coverage query selected`、`Daily Ops review entry opened` 或 `Personal/team review entry opened` 状态栏反馈。该能力只改进交接链接的人工操作反馈，不自动入账复核、不修改账本、不签名、不运行流水线、不连接券商、不提交订单，也不放宽 paper-only/live-blocked 边界。
...
Batch 85 已把 Audit 来源侧的 Coverage next 标签本体也改成 action-specific：顶部摘要和本地复核 row 会复用 `localReviewCoverageNextActionLabel`，把 next-action query 中解析出的 Daily Ops / 个人小团队缺口显示为“Daily Ops 复核缺失 / Daily Ops review missing”或“个人/小团队复核缺失 / Personal/team review missing”。底层 `localReviewBundleCoverageNextActionLabel/Query/Title` 仍作为搜索、tooltip 与深链上下文保留，UI 只是翻译可见标签，不自动记录复核、不修改账本、不签名、不运行流水线、不连接券商、不提交订单，也不放宽 paper-only/live-blocked 边界。
...
Batch 92 已收紧本地复核覆盖 next-action 深链解析：`buildLocalReviewCoverageNextActionUrlSearch` 与 `resolveLocalReviewCoverageNextActionDeepLinkState` 会拒绝未知 action 或缺少缺口 token 的查询，只有 Daily Ops 与个人/小团队两类已知人工复核动作能生成标准化 URL 或首页落地状态。这样坏交接链接不会显示为 `unknown` next action；该能力只保护只读导航上下文，不自动记录复核、不修改账本、不签名、不运行流水线、不连接券商、不提交订单，也不放宽 paper-only/live-blocked 边界。
