...
- 策略工坊已经加入草稿就绪闸门，保存或审计前会显示策略结构、风控参数、执行模式和审计证据是否通过；这些 gate 已经有本地核心 `/api/strategies/validate` 契约，前端优先使用核心校验、核心不可用时本地兜底。默认草稿的风控文本已经与结构化编辑器保持一致，研究流水线启动前也会复用核心校验，阻断未结构化或风控不完整的草稿进入回测、AI 评审或模拟执行。
