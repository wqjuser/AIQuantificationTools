...
- `POST /api/audit/events`：保存一条通用审计事件，当前用于导入复现包的预检、阻断、取消、失败和确认写入流水；响应返回规范化后的 `event`。
- `GET /api/audit/events?eventType=research_run_import&runId=...&limit=20&offset=0&query=`：按事件类型、run id 和全文 query 读取审计事件分页，返回 `events` 与 `pagination`，供 Audit 工作区恢复最近导入审计流水。
