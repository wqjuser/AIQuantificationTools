...
- Audit 签名 Key 注册表面板现在会回读最近 5 条 `audit_signing_key_rotation_plan` 账本事件，把轮换计划是否 prepared/blocked、拟启用 key、legacy 模板短 hash、阻断原因、环境变量数量和人工步骤数量显示为紧凑历史行；生成计划入账成功后会立即合并到历史列表，形成“生成计划 -> 写入账本 -> 可追溯查看”的闭环。
- Audit 签名 Key 注册表面板继续补了“应用预检”阶段：用户必须显式确认新 secret 已在本地安全保存、当前 secret 已写入 legacy 注册表、操作员已复核 key/指纹/重启影响，前端才会调用 `/api/audit/signing-keys/rotation-apply`。后端只做只读安全预检，缺确认时返回 blocked，确认齐全时返回 ready_for_restart；结果会被压缩成 `audit_signing_key_rotation_apply` 事件写入审计账本，仍不在 UI 或 API 中传输 raw secret，也不自动写环境变量或重启容器。下一步是把这个 preflight 接到真正的本地 secret-store 写入和受控重启编排。
