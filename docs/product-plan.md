...
- 组合级回测已经开始进入后端核心：`PortfolioBacktestEngine` 可以把多个已审计单标的 `BacktestRun` 按目标权重聚合为组合权益曲线、现金缓冲、组合收益/回撤/交易数、数据质量和标的贡献；本地核心新增 `POST /api/portfolio/backtest`，输入已存在的审计 run id 与权重，输出组合级证据。当前版本只做时间戳对齐的静态权重组合聚合，不做再平衡、组合交易流水、相关性分析或自动调仓。
...
- 数据源配置不是一等页面。
- 回测证据已经开始成为核心资产，参数扫描已从单纯 SMA 窗口推进到 SMA+RSI 确认阈值和 SMA+VOL 窗口组合，并有当前排名、复审候选、风险行数摘要、同上下文运行对比矩阵、导出报告里的同市场同周期跨标的横向比较，以及后端审计 run 驱动的静态权重组合回测 MVP；后续还需要补更正式的多参数扫描、组合再平衡、组合交易流水、相关性/敞口诊断和报告模板。
...
- 组合敞口和回撤。
- 跨策略资金分配。
- 组合回测；先从已审计 run 的静态权重聚合开始，再推进到再平衡、交易流水、敞口和风险贡献。
- 交易前风控引擎。
- 风控规则编辑器。
- 紧急停止和人工覆盖。
