...
Stage 9 默认 Docker acceptance 不使用宿主生产凭据或真实生产网络，覆盖无凭据 API 失败前后零制品、确定性 ready、规则/行情/价格/资金故障、连续性漂移、候选过期、候选/复核重复请求幂等、revoke 网络前阻断、detached 阻断和 API 重启 hash 精确回读；线程化回归测试另行覆盖并发幂等。Stage 4→9 准入权威链只能由专用 API 写入，通用审计入口不能创建、预占或覆盖权威事件，研究包也不能恢复 Stage 7 probe 或 Stage 8 control；回读会重验事件绑定。可选真实只读门禁用实际 CCXT 方法守卫验证只读调用，只接受 ready 或纯资金不足安全阻断，并在重启后回读。2026-07-14 最新 accepted manifest hash 为 `d3c53eccd6e3689ddfde808dca1b1d68f90abda4af62268b7fd60347248605c3`。退出不要求生产账户入金；生产订单创建、查询、撤销、成交、转账、提现和 live route 仍不存在。
...
后续开发必须按“大批次垂直闭环”推进，而不是继续堆零散字段、局部闸门或孤立面板。P0 执行基线以本文件的路线与最新进展为准：先完成 P0 Golden Path Workbench，再依次推进 A 股数据就绪、策略到回测流水线、AI 评审、纸面交易、复现包和产品化收口。除非某个小修复直接阻断这些批次，否则不再单独推进零散改动。
...
- 历史 K 线拉取和缓存状态。
- 数据质量告警。
- A 股全市场、Binance USDT 现货和美股自选池的可审计 AI 选股；先按确定性基本面、趋势和流动性证据筛选，再允许 AI 在合格候选中重排，只生成逐只研究入口。达到固定持有周期后，用已完成日 K 和同周期审计基准复盘收益，并按明确样本量汇总命中率、候选合格率、主要排除原因、数据源降级率、AI 成功率和风格历史表现；样本不足时不启用批量研究、自动观察池或任何订单连接。
- 多个待映射交易对时，CoinGecko Binance 交易对映射先用官方 coin ID 列表按 base symbol 取全部 ID 候选，再用 Binance ticker 端点的 `coin_ids` 过滤结果验证精确交易对；同 symbol 的 ID 不做单选，同一交易对多 ID 命中仍按歧义失败关闭。单一交易对或 ID 列表不可用时使用既有全量分页扫描；分页或市场事实源限流时保留已验证结果，同一进程内的后续显式选股从未完成页继续。可能跨页的边界交易对与其余未覆盖候选保持未解析或市场事实缺失；续扫页最小交易对回退到旧排序锚点之前时从第一页重核。不再以通用 `502` 丢失已取得的精确映射，也不使用同名推断补齐覆盖；没有后台重试器或新持久化状态。
- A 股双源冲突继续失败关闭，并在既有候选排除原因中区分具体事实字段、来源独立性、货币单位和报告期问题；质量统计直接回放这些原因，不新增冲突表或第二套诊断状态。
- 美股 SEC Company Facts 使用平台配置中的真实联系 User-Agent，并在保存后热加载。实网自选池验收必须同时证明新鲜报价、官方 ticker/CIK 映射、180 根已完成日 K、点时财务期间、规范哈希和研究-only 边界；“仅覆盖当前自选池”是范围声明，不计入数据源降级。
- API Key 和数据源配置。
- 本地缓存检查、刷新和修复工具。
...
- 研究运行审计库已经存在。
- 策略配置和回测合同已经存在。
- 策略库已经不再只是当前标的的小列表；最近策略版本可在 Strategy Lab 全局查看，并显示与当前草稿的上下文、名称、入场、出场、仓位和风控差异；载入时会切换到该版本绑定的市场、标的和周期，并清空旧审计证据，要求重新运行流水线。研究运行导入会把包内 `researchRun.strategyConfig` 按原始 revision 还原为 audited 策略版本，前端导入完成后也会刷新同上下文策略库并合并到 Strategy Lab 最近策略列表，避免跨机器导入后策略工坊和审计 run 断链。每个已保存版本都支持通过主题化确认弹窗删除，包括当前已载入、已审计和最后一个版本；删除只移除本地策略库记录，当前草稿、历史研究运行、策略实验和审计证据继续保留。
...
- 行情中心已经开始承担 watchlist 数据准备职责：当前周期下会显示自选缓存 fresh/stale/empty 摘要；一键刷新自选列表缓存现在会调用核心 `POST /api/cache/watchlist-refreshes`，生成本地 SQLite 持久化的 watchlist cache refresh run，记录 run id、可选人工覆盖审计事件 id、每个标的的数据质量、入库行数、跳过/失败原因和最新 Settings 状态；批量刷新会以最多四个 worker 并发执行外部 K 线读取，并只在这些 worker 内把公共 K 线 HTTP 交互超时限制为 3 秒，图表、研究流水线和单标的刷新仍保留 10 秒默认值；读取结束后仍按自选顺序落库和记录证据，避免慢数据源把等待逐项累加且不改变 fallback、质量标记或 run 顺序；前端刷新设置时会通过 `GET /api/cache/watchlist-refreshes?limit=4` 回读最近运行，数据源健康面板会同时显示最近一次摘要、最近多次刷新历史和选中运行的逐标的状态明细，点击历史 run 可切换明细，点击明细行可切换到对应研究上下文，页面刷新或 Docker 重启后仍能看到自选数据准备证据链。
...
- Stage 0 Python 开发环境现在由 `services/quant_core/pyproject.toml` 单点声明：本机建立 `.venv` 后以 editable 模式安装，跨平台启动器优先使用该环境；CI 复用相同安装入口，不再手写 `cryptography` 依赖。`quant_core` 显式作为唯一安装包，避免运行时 SQLite `data/` 被 setuptools 误识别。完整测试前新增 AI 选股与 Stage 10 聚焦安全门禁，只增强失败定位与回归可见性，不新增交易能力或第二套测试框架。
...
- Stage 0 P1 验收工作量已固定为工作区顺序中的前三个标的：工作区仍必须至少包含三个标的并如实报告原始数量，但 refresh payload、queue-ready 选择和 `aiqt.p1AcceptanceManifest` 只复用同一份三标的样本。该验收采样收口避免产品自选增长无界放大 CI 的外部请求工作量，不缩减产品完整 watchlist refresh，也不改变 90 秒端到端预算或 live-blocked 边界；产品刷新链路后续采用的四路有界读取与批量 worker 3 秒公共源交互超时由同一 API 实现承担，其他行情路径继续使用 10 秒默认值。
...
- Stage 1 研究上下文就绪清单和研究 run 快照现在纳入“交易日历证据”：前端会把当前市场交易日历作为 `calendar` 就绪行展示；`open / always_open` 且没有其他异常时显示就绪，静态时段模板自带的“未配置交易所节假日日历”提示仍保留在详情与审计证据中，但不再让正常开市进入确认弹窗。休市、盘中休市、未知状态或其他日历 warning 仍进入 review gate；后端 `/api/research/run` 会把同一时刻的 `marketCalendar` 写入 `researchRun.dataSnapshot` 并随审计 run 持久化。该证据只用于研究复核和后续导出链路，不解锁模拟委托、真实委托或任何交易路由。
- Stage 1 黄金路径现在也会解释运行前交易日历复核：`/api/golden-path/status` 在当前上下文尚未生成审计 run 时，会把需要复核的市场交易日历作为 `market-data` 步骤的附加 review 原因；缓存缺失、空缓存、过期缓存和刷新证据不完整仍优先提示刷新数据。缓存与刷新证据可用且当前为 `open / always_open` 时，单独的静态节假日覆盖提示不会让步骤倒退；休市、盘中休市、未知状态或其他日历 warning 仍把下一步指向 Research 的运行流水线，由研究页复核确认承接。已有审计 run 后，黄金路径继续使用 run 中锁定的日历证据流转，不因当前时刻静态日历提示倒退主流程。
...
最新更新：行情中心完成桌面、超宽屏与 375px 专项细节复审。桌面八列自选表使用固定列宽并保持在面板内；当真实自选不足八条时，下方复用现有自选与报价字段展示上涨、下跌、覆盖市场、当前标的和更新时间概览，不补造行情行。页面标题、搜索、市场选择、高级证据入口和刷新动作现在共用紧凑工具行；概览卡片使用剩余高度，K 线面板按实际标题高度弹性分配正文，底部标的、收盘、日期、数据源和 K 线数量不再被裁切。移动端将同一张表投影为代码、名称、最新价和涨跌幅四个核心字段，并把高级证据入口移到标题操作行，避免覆盖市场选择器。Docker 复验确认桌面图表溢出 0px、1440px 与 375px 页面均无横向溢出；交易安全边界未改变。

最新更新：研究流水线运行前复核弹窗已统一使用中文显示确认项、状态和处理说明，同时为标的、自选、交易日历、K 线/缓存、刷新证据、研究笔记和工作区状态提供明确的直达按钮。直达动作只切换既有工作区、滚动并聚焦真实控件，不会自动保存或刷新；新版研究工作台补回了复用现有研究笔记 store/API 和工作区持久化 API 的“研究准备”区，行情中心补回显式自选保存入口。底层审计与导出继续保留原始英文域值，中文只在 UI 展示层转换，因此不改变历史证据兼容性、研究运行语义或任何交易安全边界。

最新更新：研究工作台的“研究准备”现在支持 AI 生成中文结构化研究笔记草稿。实现复用 Stage 3 既有 `local/openai/openai-compatible/ollama` Provider registry、输出 schema、安全语义校验和行情缓存，不建立第二套模型接入；外部 Provider 每次生成前都要单独勾选授权，授权在请求发出时即被消费，并在页面展示 model、脱敏 Base URL 和允许外发的字段范围。仅发送市场、标的、周期、缓存范围、行数和派生统计，不发送原始 K 线或已有研究笔记。生成结果只替换当前编辑草稿，慢响应在上下文变化或用户曾经编辑草稿（包括编辑后撤销回原文）时会被丢弃，只有用户点击“保存研究笔记”才写入既有 store。外部失败、不配置、超出 60 秒墙钟总预算或输出不是合规中文研究内容时，后端仍返回确定性本地基线作为安全回退证据；当前草稿为空时前端采用该基线，已有用户内容时则保留原文，并提示重新授权重试或显式切换“本地基线”生成。`paperOnly/liveTradingAllowed/orderSubmissionAllowed` 边界保持不变。

最新更新：AI 研究笔记草稿已从“完整生成后分段播放”升级为真实上游正文流。OpenAI Responses、OpenAI-compatible 和 Ollama 在同一次请求内流式返回结构化 assessment；服务端不把原始 JSON token 交给浏览器，而是从可解析字段片段持续构建只增不减的 Markdown，在每次下发前检查协议、字段类型、禁止交易语义、事件数量和累计字节，再通过同一个 `POST /api/research/note-drafts` NDJSON 响应发送 `draft`。兼容 Provider 的严格 `json_schema` 模式实测会被当前网关缓冲，因此只有研究笔记流请求改为把完整 schema 写入 prompt 并使用 `json_object`，非流式 AI 评审仍保留严格 `json_schema`；最终结构、证据和安全校验完全不变。仅研究笔记的 OpenAI-compatible 流请求同时关闭可选推理强度以降低首段延迟，不支持该参数时会在正文尚未开始前、同一总预算内安全重试。Provider 流式信封与最终 JSON 分别使用 524,288 bytes 和 65,536 字符上限；对浏览器的累计草稿事件另限制为 240 次、250,000 线缆字节，外部成功响应限制为 900,000 线缆字节，浏览器再以 512 个事件和 1,000,000 接收字节闭合协议。Web 端只保留一个研究笔记 textarea，累计草稿直接写入该编辑器，生成期间禁止保存；完成后重新执行完整 assessment schema、中文、证据引用、Provider 身份和跨字段安全校验，只解除生成态而不再次搬运正文。后续字段违规、断流、超限、上下文变化、用户编辑或缺少完成事件都会恢复生成前内容，空草稿的外部失败继续使用精确的确定性本地基线。外部授权、不自动保存和 `paperOnly/liveTradingAllowed/orderSubmissionAllowed` 边界均未改变。

最新更新：策略工坊新增 AI 帮写结构化策略草稿。用户先用自然语言描述目标，系统把当前市场、标的、周期和完整 `StrategyRuleDraft` 交给所选 Provider，返回可直接映射到既有编辑器的候选参数，并同步生成 3 至 6 条中文“为什么这样编写”，覆盖信号选择、确认过滤、仓位和风险约束。该能力复用 Stage 3 既有 Provider registry、输出校验、密钥隔离和失败回退，不保留旧的固定 SMA 伪生成分支，也不新增策略模型、Store 或执行算法。外部 Provider 每次生成前必须明确授权，允许外发的字段固定为市场、标的、周期、目标和当前结构化草稿；原始 K 线、账户、订单、密钥和审计结论不会外发。服务端严格校验候选结构、枚举、范围、中文原因、Provider 身份和禁止交易语义，并复用现有策略 readiness validator；前端继续精确核对候选与校验配置的一致性。候选预览逐项展示所有发生变化的“当前值 → 候选值”和编写原因；标的、周期或草稿变化会使旧候选失效，生成请求可主动取消并在 45 秒后超时。用户点击“应用到当前草稿”后才替换编辑值，同时清除旧审计、回测与指标绑定；保存版本和重新审计仍是后续独立动作。生成、预览和应用均保持 `paperOnly=true`、`liveTradingAllowed=false`、`orderSubmissionEnabled=false`、`routeExecuted=false`。

最新更新：M4 AI 研究闭环已完成。权威 AI 评审现在可以生成只追加的 `aiqt.aiResearchEvidence`：事实、计算、假设和模型推断分别标记并保留引用，信息丰富度与投资确定性独立计算；A 股六类重要财务事实要求两个独立来源观测，报告期、单位和差异被分类但绝不合并数值；日线或周线可选展示看多、看空、中性三视角，分钟级服务端阻断。声明周期到期后，系统只消费同上下文已审计研究运行和独立基准运行，记录原始收益、观点调整收益、最大不利波动、基准收益和 alpha，并绑定原建议与三份快照。历史结果只进入后续研究上下文，`affectsRisk/affectsAuthorization/affectsPermissions/affectsOrderRouting` 始终为 `false`。实现复用 Stage 3 评审、研究运行和 `AuditEventStore`，没有新增 Provider、Agent 编排、交易状态机、数据库表或订单路由。

最新更新：M5 组合风险闭环已完成。组合工作区从同一份已审计 Stage 4 工作流和纸面账户回放同时展示当前权重、原目标、风险调整目标、漂移、现金和候选调仓，并按行业、市场和币种聚合当前/目标/调整后暴露；相关性与协方差风险贡献直接复用现有组合回测。账户未匹配持仓、最大回撤、当日损失、交易频率、相关性或风险贡献越限会阻断整个候选批次；总暴露和集中度限额只能缩减目标，不能增加任何标的暴露，缩减部分回到现金。评估以 `aiqt.portfolioRiskAssessment` 写入既有 `AuditEventStore` 并幂等回读，不新增组合引擎、状态机、数据库表或订单路由；页面动作只生成 paper-only 风险评估，`liveTradingAllowed=false`、`orderSubmissionEnabled=false`、`routeExecuted=false` 和 `liveBlockedBoundary=true` 保持不变。
