...
import { Status, SurfacePanel } from "./TerminalSurfaceUi";
...
export function PortfolioM5Section({
  ...
  return (
    <SurfacePanel
      ...
      className="portfolio-m5-section"
      subtitle="读取阶段 4 模拟账户回放；该评估不写入生产风险链，只生成研究风险证据，不审批、不提交或路由订单"
      title="高级组合风险评估"
    >
      <details className="portfolio-m5-disclosure" open={Boolean(error || busy) || undefined}>
        <summary>
          <span>账户、目标与批次风险</span>
          <small>设置、运行与查看审计结果</small>
        </summary>
        <div className="portfolio-m5-content">
          ...
          {assessment ? (
            ...
            <div className="portfolio-m5-empty">
              Stage 4 账户回放已就绪，可在此生成可审计的组合风险评估。
            </div>
          )}
        </div>
      </details>
