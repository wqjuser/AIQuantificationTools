...
## 2026-08-14 第三方 AI 连接弹窗复验

- 参考图为用户提供的侧栏账户卡片截图；实现保留原账户卡片结构，仅把 Claude 专属链接改为“连接第三方 AI”按钮，并在工作台内打开原生对话框。
- `1280 × 720` 已认证公网会话状态下，弹窗同时显示 Claude 一键连接、Claude Code 与 Codex CLI 命令；打开前后 URL 不变，关闭后原行情工作区仍在，弹窗高度受视口约束并可独立滚动。
- 浏览器控制台错误为 `0`；交易边界与八个只读研究工具保持不变。桌面浏览器验收无 P0/P1/P2 遗留。
...
- 对照实验列表不再固定显示条数。桌面端“评审结论”和“对照实验”共用 `343px` 面板高度，Docker 亮色页面实测均为 `343px`、差值 `0`；`10` 个真实候选在剩余 `270px` 正文区域内滚动，`max-height=100%`、`scrollHeight=524`。窄屏恢复自然高度，页面不会继续拉长右栏或产生横向溢出。
...
## 2026-08-01 A 股双源冲突原因可观测性复验

- 现有 4 次 A 股受保护选股审计共包含 `28` 个合格候选，全部为新浪/东方财富双源 `verified`；另以真实公开接口复验最近候选 `300308`、`688256`、`300502`，两源的报告期、币种、单位及六项财务事实均一致。因此保留既有 `1%` 容差，没有为制造冲突而修改真实数据判定。
- 修复前，六项财务事实不一致也会被笼统记录为“单位或报告期冲突”，质量统计无法给出真实主要原因。现在复用既有 exclusion reason，分别记录具体不一致字段、来源不独立、单位缺失、单位不一致和报告期不一致；受保护选股审计与质量统计回放同一中文原因，不新增字段、数据库表或诊断状态机。
- API 数据依赖镜像重建后，真实 A 股全市场选股返回 `201 / partial`：`7` 个合格候选、`5` 个推荐和 `93` 个排除；7 个候选均为新浪/东方财富双源 `verified`。当前质量统计覆盖 `6` 次选股、`44 / 600` 个合格候选，已复盘样本仍为 `0`，因此没有推进评分优化、批量研究、自动观察池或订单连接。
...
## 2026-08-01 CoinGecko Binance 精确映射实网验收

- 待五分钟负缓存过期后，通过现有条件筛选公开 API 执行 `BTC` 且最低成交额 `100,000,000 USDT` 的 Binance USDT 实网选股；请求返回 `201 / completed`，生成 `selection-v2-d9cdb722d0c556670371`，只有 `BTC/USDT` 进入合格候选与研究推荐，无排除项。
- 受保护审计事件记录 `BTC/USDT → bitcoin` 精确映射，本次预筛候选映射覆盖为 `1 / 1 · 100%`，歧义、缺失与未解析均为 `0`；CoinGecko 与 Binance 事实都保留来源及观测时间，没有同名推断或浏览器上传事实。
- 候选证据冻结 `180` 根已完成日 K（`2026-02-02` 至 `2026-07-31`）、技术因子、流动性与供应事实、市场快照哈希、记录哈希和候选证据哈希。本地确定性 Provider 状态为 `skipped`，未对外发送数据；边界仍为 `researchOnly=true`、`orderSubmissionAllowed=false`、`routeExecuted=false`。
