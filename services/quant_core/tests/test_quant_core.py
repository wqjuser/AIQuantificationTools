...
class QuantCoreContractTest(unittest.TestCase):
    ...
    def test_execution_adapter_orchestration_execution_records_controlled_handoff_without_enabling_live(self):
            ...
            try:
                ...
                plan_response, plan_payload = post_json(
                    "/api/execution/adapter-runtime-reload-plans",
                    {
                        ...
                        "operator": "runtime-operator",
                        "reloadMode": "manual_container_reload_plan",
                        "maintenanceWindowId": "window-ccxt-orchestration-1",
                        "confirmations": {
                            "maintenanceWindowApproved": True,
                            "healthBaselineCaptured": True,
                            "configDiffReviewed": True,
                ...
                missing_dry_run_response, missing_dry_run_payload = post_json(
                    "/api/execution/adapter-orchestration-executions",
                    {
                        "adapterId": "ccxt-live",
                        "dryRunId": "missing-orchestration-dry-run",
                        "operator": "runtime-operator",
                        "confirmations": {},
                    },
                )
                ...
                blocked_response, blocked_payload = post_json(
                    "/api/execution/adapter-orchestration-executions",
                    {
                        ...
                        "metadata": {
                            "source": "settings-panel",
                            "apiKey": "orchestration-execution-blocked-api-key-should-not-leak",
                        },
                    },
                )
                ...
                recorded_response, recorded_payload = post_json(
                    "/api/execution/adapter-orchestration-executions",
                    {
                        ...
                        "metadata": {
                            "source": "settings-panel",
                            "privateKey": "orchestration-execution-private-key-should-not-leak",
                        },
                    },
                )

                connection.request("GET", "/api/execution/adapter-orchestration-executions?adapterId=ccxt-live&limit=5")
                history_response = connection.getresponse()
                history_payload = json.loads(history_response.read().decode("utf-8"))
            finally:
                connection.close()
        ...
        self.assertEqual(missing_dry_run_response.status, 404)
        self.assertEqual(missing_dry_run_payload["error"], "execution_adapter_orchestration_dry_run_not_found")
        self.assertEqual(blocked_response.status, 409)
        self.assertEqual(blocked_payload["adapterOrchestrationExecution"]["status"], "blocked")
        self.assertEqual(
            blocked_payload["adapterOrchestrationExecution"]["blockedReasons"],
            [
        ...
        self.assertEqual(recorded_response.status, 201)
        self.assertEqual(recorded_payload["adapterOrchestrationExecution"]["status"], "execution_recorded")
        self.assertEqual(recorded_payload["adapterOrchestrationExecution"]["dryRunId"], dry_run_id)
        self.assertEqual(recorded_payload["adapterOrchestrationExecution"]["adapterId"], "ccxt-live")
        self.assertEqual(
            recorded_payload["adapterOrchestrationExecution"]["orchestrationExecutionMode"],
            "manual_adapter_orchestration_execution",
        )
        self.assertEqual(recorded_payload["adapterOrchestrationExecution"]["requiredEnvVars"], ["CCXT_API_KEY", "CCXT_API_SECRET"])
        self.assertFalse(recorded_payload["adapterOrchestrationExecution"]["liveTradingAllowed"])
        self.assertTrue(recorded_payload["adapterOrchestrationExecution"]["paperOnly"])
        self.assertEqual(recorded_payload["auditEvent"]["eventType"], "execution_adapter_orchestration_execution")
        self.assertEqual(history_response.status, 200)
        self.assertEqual(len(history_payload["adapterOrchestrationExecutions"]), 2)
        self.assertEqual(history_payload["adapterOrchestrationExecutions"][0]["status"], "execution_recorded")
    ...
    def test_execution_adapter_sandbox_probe_plan_records_next_gate_without_enabling_live(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            human_confirmation = build_execution_adapter_human_confirmation(
                ...
                operator="human-operator",
                metadata={"source": "settings-panel"},
                human_confirmation_id="execution-adapter-human-confirmation-ccxt-live",
            )
            ...
            try:
                missing_response, missing_payload = post_json(
                    "/api/execution/adapter-sandbox-probe-plans",
                    {
                        "adapterId": "ccxt-live",
                        "humanConfirmationId": "missing-human-confirmation",
                ...
                blocked_response, blocked_payload = post_json(
                    "/api/execution/adapter-sandbox-probe-plans",
                    {
                        ...
                        "metadata": {
                            "source": "settings-panel",
                            "apiKey": "sandbox-probe-blocked-api-key-should-not-leak",
                        },
                    },
                )
                ...
                recorded_response, recorded_payload = post_json(
                    "/api/execution/adapter-sandbox-probe-plans",
                    {
                        ...
                        "metadata": {
                            "source": "settings-panel",
                            "privateKey": "sandbox-probe-private-key-should-not-leak",
                        },
                    },
                )

                connection.request("GET", "/api/execution/adapter-sandbox-probe-plans?adapterId=ccxt-live&limit=5")
                history_response = connection.getresponse()
                history_payload = json.loads(history_response.read().decode("utf-8"))
            finally:
                connection.close()
        ...
        self.assertEqual(missing_payload["error"], "execution_adapter_human_confirmation_not_found")
        self.assertEqual(blocked_response.status, 409)
        self.assertEqual(blocked_payload["adapterSandboxProbePlan"]["status"], "blocked")
        self.assertEqual(
            blocked_payload["adapterSandboxProbePlan"]["blockedReasons"],
            [
        ...
        self.assertEqual(recorded_response.status, 201)
        self.assertEqual(recorded_payload["adapterSandboxProbePlan"]["status"], "probe_plan_recorded")
        self.assertEqual(
            recorded_payload["adapterSandboxProbePlan"]["humanConfirmationId"],
            "execution-adapter-human-confirmation-ccxt-live",
        )
        self.assertEqual(recorded_payload["adapterSandboxProbePlan"]["adapterId"], "ccxt-live")
        self.assertEqual(recorded_payload["adapterSandboxProbePlan"]["probeMode"], "manual_sandbox_probe_plan")
        self.assertEqual(recorded_payload["adapterSandboxProbePlan"]["requiredEnvVars"], ["CCXT_API_KEY", "CCXT_API_SECRET"])
        self.assertFalse(recorded_payload["adapterSandboxProbePlan"]["liveTradingAllowed"])
        self.assertTrue(recorded_payload["adapterSandboxProbePlan"]["paperOnly"])
        self.assertEqual(recorded_payload["auditEvent"]["eventType"], "execution_adapter_sandbox_probe_plan")
        self.assertEqual(history_response.status, 200)
        self.assertEqual(len(history_payload["adapterSandboxProbePlans"]), 2)
        self.assertEqual(history_payload["adapterSandboxProbePlans"][0]["status"], "probe_plan_recorded")
        self.assertEqual(history_payload["adapterSandboxProbePlans"][1]["status"], "blocked")
    ...
    def test_execution_adapter_sandbox_probe_execution_records_readonly_probe_without_enabling_live(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                missing_response, missing_payload = post_json(
                    "/api/execution/adapter-sandbox-probe-executions",
                    {
                        "adapterId": "ccxt-live",
                        "sandboxProbePlanId": "missing-sandbox-probe-plan",
                        "operator": "sandbox-operator",
                        "confirmations": {},
                    },
                )
                ...
                blocked_response, blocked_payload = post_json(
                    "/api/execution/adapter-sandbox-probe-executions",
                    {
                        ...
                        "metadata": {
                            "source": "settings-panel",
                            "apiSecret": "sandbox-probe-execution-blocked-secret-should-not-leak",
                        },
                    },
                )
                ...
                recorded_response, recorded_payload = post_json(
                    "/api/execution/adapter-sandbox-probe-executions",
                    {
                        ...
                        "metadata": {
                            "source": "settings-panel",
                            "privateKey": "sandbox-probe-execution-private-key-should-not-leak",
                        },
                    },
                )

                connection.request("GET", "/api/execution/adapter-sandbox-probe-executions?adapterId=ccxt-live&limit=5")
                history_response = connection.getresponse()
                history_payload = json.loads(history_response.read().decode("utf-8"))
            finally:
                connection.close()
        ...
        self.assertEqual(missing_response.status, 404)
        self.assertEqual(missing_payload["error"], "execution_adapter_sandbox_probe_plan_not_found")
        self.assertEqual(blocked_response.status, 409)
        self.assertEqual(blocked_payload["adapterSandboxProbeExecution"]["status"], "blocked")
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "execution-adapter-sandbox-probe-plan-ccxt-live",
        )
        self.assertEqual(recorded_payload["adapterSandboxProbeExecution"]["adapterId"], "ccxt-live")
        self.assertEqual(
            recorded_payload["adapterSandboxProbeExecution"]["probeExecutionMode"],
            "manual_readonly_sandbox_probe",
        )
        self.assertEqual(
    ...
    def test_execution_adapter_production_route_review_records_policy_attestation_without_enabling_live(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            sandbox_probe_plan = build_execution_adapter_sandbox_probe_plan(
                {
                    ...
                    "executionMode": "manual_controlled_reload",
                    "reloadMode": "manual_container_reload_plan",
                    "maintenanceWindowId": "window-ccxt-production-route-review-1",
                    "bindingMode": "container_env_reference",
                    "manifestPath": "local-secret-store://ccxt-live/sandbox",
                    "requiredEnvVars": ["CCXT_API_KEY", "CCXT_API_SECRET"],
                },
            ...
            sandbox_probe_review = build_execution_adapter_sandbox_probe_review(
                {
                    **sandbox_probe_execution.__dict__,
                    "sandboxProbeExecutionId": sandbox_probe_execution.sandbox_probe_execution_id,
                    "sandboxProbePlanId": sandbox_probe_execution.sandbox_probe_plan_id,
                    "humanConfirmationId": sandbox_probe_execution.human_confirmation_id,
                    "orchestrationExecutionId": sandbox_probe_execution.orchestration_execution_id,
                    "dryRunId": sandbox_probe_execution.dry_run_id,
                    "acceptanceId": sandbox_probe_execution.acceptance_id,
                    "executionId": sandbox_probe_execution.execution_id,
                    ...
                    "probeMode": sandbox_probe_execution.probe_mode,
                    "confirmationMode": sandbox_probe_execution.confirmation_mode,
                    "orchestrationExecutionMode": sandbox_probe_execution.orchestration_execution_mode,
                    "orchestrationMode": sandbox_probe_execution.orchestration_mode,
                    "acceptanceMode": sandbox_probe_execution.acceptance_mode,
                    "executionMode": sandbox_probe_execution.execution_mode,
                    "reloadMode": sandbox_probe_execution.reload_mode,
                    "maintenanceWindowId": sandbox_probe_execution.maintenance_window_id,
                    "bindingMode": sandbox_probe_execution.binding_mode,
                    "manifestPath": sandbox_probe_execution.manifest_path,
                    "requiredEnvVars": sandbox_probe_execution.required_env_vars,
                    "liveTradingAllowed": False,
                    "paperOnly": True,
                },
                adapter_id="ccxt-live",
            ...
            try:
                missing_response, missing_payload = post_json(
                    "/api/execution/adapter-production-route-reviews",
                    {
                        "adapterId": "ccxt-live",
                        "sandboxProbeReviewId": "missing-sandbox-probe-review",
                ...
                blocked_response, blocked_payload = post_json(
                    "/api/execution/adapter-production-route-reviews",
                    {
                        ...
                        "metadata": {
                            "source": "settings-panel",
                            "apiSecret": "production-route-review-blocked-secret-should-not-leak",
                        },
                    },
                )
                ...
                recorded_response, recorded_payload = post_json(
                    "/api/execution/adapter-production-route-reviews",
                    {
                        ...
                        "metadata": {
                            "source": "settings-panel",
                            "privateKey": "production-route-review-private-key-should-not-leak",
                        },
                    },
                )

                connection.request("GET", "/api/execution/adapter-production-route-reviews?adapterId=ccxt-live&limit=5")
                history_response = connection.getresponse()
                history_payload = json.loads(history_response.read().decode("utf-8"))
            finally:
                connection.close()
        ...
        self.assertEqual(missing_response.status, 404)
        self.assertEqual(missing_payload["error"], "execution_adapter_sandbox_probe_review_not_found")
        self.assertEqual(blocked_response.status, 409)
        self.assertEqual(blocked_payload["adapterProductionRouteReview"]["status"], "blocked")
        self.assertEqual(
            blocked_payload["adapterProductionRouteReview"]["blockedReasons"],
            [
        ...
        self.assertEqual(recorded_response.status, 201)
        self.assertEqual(recorded_payload["adapterProductionRouteReview"]["status"], "route_review_recorded")
        self.assertEqual(
            recorded_payload["adapterProductionRouteReview"]["sandboxProbeReviewId"],
            "execution-adapter-sandbox-probe-review-ccxt-live",
        )
        self.assertEqual(recorded_payload["adapterProductionRouteReview"]["adapterId"], "ccxt-live")
        self.assertEqual(recorded_payload["adapterProductionRouteReview"]["reviewMode"], "manual_production_route_review")
        self.assertEqual(
