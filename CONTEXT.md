...
**决策提案**：
规则、风险或已记录 AI 响应针对一个市场快照给出的原始方向、置信度和理由。AI 提案是审计输入，不是可重复调用的确定性函数，也不能绕过后续边界。
_Avoid_: AI 直接订单、覆盖原始提案、把提案当最终信号

**标准信号**：
决策提案经过当前确定性持仓边界后形成的买入、卖出或持有结论。自动交易与回测共享同一条持仓边界规则：已有持仓不重复买入，空仓不可卖出；回测结束时的强制平仓只属于模拟结算。提案与信号可以不同，但必须通过提案 ID、快照哈希和策略修订保持可追溯。
...
**订单意图**：
风险调整目标存在非零批准差额时生成的确定性市价委托描述，绑定市场快照、策略修订、提案、信号、组合目标和风险调整目标，并记录标的、方向、数量、参考价与名义金额。相同证据必须生成相同订单意图 ID；无批准差额时不生成订单意图。测试网或生产路由的稳定 `clientOrderId` 从执行模式和订单意图 ID 派生。
_Avoid_: 持有信号空委托、路由重新计算数量、订单脱离决策证据、时间戳充当订单身份

**订单结果**：
纸面、测试网或生产实盘对一个订单意图产生的统一执行事实，必须绑定原订单意图 ID，并记录执行模式、订单状态、成交数量、剩余数量、成交均价与成交金额。测试网和生产结果由既有路由回执及只读对账推进；纸面结果使用同一字段语义但不伪造外部订单号。
