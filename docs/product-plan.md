...
5. 将研究笔记纳入导出包和 AI 评审证据边界。
6. 建立 Backtest Report 页面，展示指标、交易流水、权益曲线、诊断和 AI 评审准备状态。
7. 将模拟交易移动到 Execution Center，以委托生命周期和账户状态为主。
8. 增加 Settings，用于数据源、API Key、缓存、执行适配器认证。
