...
- AI 评审已经开始绑定审计证据，并会把同数据快照基准收益、Alpha 和参数扫描摘要纳入证据卡、评审 dossier、回测解释动作和可导出的 Markdown AI 评审报告；同时可以导出和保存结构化 AI Review Run Record JSON，记录 citation、委员会轮次、决策日志和安全边界。前端 AI Review 面板已经接入 `AiReviewRunStore` 与 `/api/research/runs/{runId}/ai-reviews` 的 POST/GET 契约，可把该记录绑定到审计 run，并在 run 回放/导入后恢复最近保存的 AI 评审记录；本地核心读取接口已经支持按 run 返回 AI Review Run Record，并提供 `limit`、`offset`、`query` 和 `pagination` 元信息，为后续审计页分页检索提供后端契约；Audit 工作区已经开始展示同一审计 run 的 AI Review Run Record 历史和引用证据，能把当前 run id、策略版本、dossier 状态、引用数量、委员会轮次、实盘边界和执行前风控审批与最近或用户选中的保存记录做轻量对照，也能汇总所有已加载保存记录相对当前证据的漂移项，并按版本、记录 ID、状态、计数、实盘边界和漂移原因进行本地过滤，且同一个过滤条件会同步作用到漂移摘要和已保存记录历史，方便在回放页确认 AI 解释是否绑定了正确数据、回测、研究笔记和风控边界；研究运行 JSON 复现包已经把已保存 AI 评审记录作为 `aiReviewRuns` artifact 导出，导入时会校验并写回目标核心的 `AiReviewRunStore`，让跨机器回放也能恢复 AI 委员会证据链；参数候选继续使用“复审候选/非投资建议”措辞，不直接给买卖建议。
...
- 导出、导入和完整性校验已经存在；manifest artifact counts 已覆盖数据快照、交易流水、研究笔记、模拟执行、晋级候选和 AI 评审记录，防止复现包宣称的证据数量和实际内容不一致。
...
- AI 评审已有 run id、数据质量、研究笔记、回测报告、基准 Alpha、参数扫描摘要证据引用、结构化 AI Review Run Record JSON、独立 Markdown 报告导出、本地核心持久化 API、前端保存/回放读取动作、后端分页检索契约、Audit 页证据轨迹、当前/最近或选中保存记录对照、多记录漂移摘要、执行前风控审批引用，以及同步作用于漂移摘要和记录历史的本地过滤；后续需要把审计页 UI 的分页控件接入该后端契约。
- 组合、风控、执行还不够像交易平台。
