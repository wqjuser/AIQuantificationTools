...
- 组合纸面委托已经从单次回测 payload 进入本地可审计记录：本地核心新增 `PortfolioPaperOrderStore` 和 `/api/portfolio/paper-orders` POST/GET 契约，可以把 `paperOrderEvents` 按 `baseRunId`、组合名称、来源、批次 id、订单数组和状态汇总持久化到 SQLite。Portfolio 工作区新增显式“入账委托”动作，并会按当前审计 run 回读最近入账批次，展示批次 id、订单数、名义金额和状态计数。绑定单一审计 run 的组合委托批次已经进入研究运行 JSON 复现包：导出包会携带 `portfolioPaperOrderBatches` 和 `artifactCounts.portfolioPaperOrderBatches`，导入时校验 base run、写回目标核心的 Portfolio 批次库，并纳入导入失败回滚和 undo 快照；Audit 包浏览器、近期复现包索引和导入预检会显示/搜索该 artifact 的 manifest/package 数量一致性。该能力仍是 paper-only 的组合执行准备账本，不连接真实券商、不生成真实成交；下一步应把这些批次并入通用审计事件聚合和执行中心的组合订单生命周期。
