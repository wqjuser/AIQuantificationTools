...
- `backtest.py` 的入场条件支持 `volume_above_sma`、`rsi_below` 和 `rsi_above`，用于把 Strategy Lab/AI draft 中的成交量确认和 RSI 阈值纳入真实回测触发，而不是只停留在文本描述。
