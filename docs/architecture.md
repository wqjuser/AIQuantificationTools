...
`buildAiReviewRunRecord` 会把已审计 run 的 AI 评审状态打包为 `aiqt.aiReviewRun` JSON：包含 run id、策略 revision、市场/标的/周期、dossier、citations、committee rounds、decision log、摘要计数和“仅解释证据”的安全边界。前端 AI Review 面板可导出该结构化记录，后续本地核心可以直接复用这一契约落库。
