...
- `POST /api/audit/events`：保存一条通用审计事件，当前用于导入复现包的预检、阻断、取消、失败和确认写入流水；导入事件 metadata 会携带旧 run、可回放恢复目标、失败分类和恢复提示，响应返回规范化后的 `event`。
