...
- AI 评审已经开始绑定审计证据，并会把同数据快照基准收益、Alpha 和参数扫描摘要纳入证据卡、评审 dossier、回测解释动作和可导出的 Markdown AI 评审报告；同时可以导出结构化 AI Review Run Record JSON，记录 citation、委员会轮次、决策日志和安全边界。本地核心已经提供 `AiReviewRunStore` 与 `/api/research/runs/{runId}/ai-reviews` 的 POST/GET 契约，用于把该记录绑定到审计 run；参数候选继续使用“复审候选/非投资建议”措辞，不直接给买卖建议。
...
- AI 评审已有 run id、数据质量、研究笔记、回测报告、基准 Alpha、参数扫描摘要证据引用、结构化 AI Review Run Record JSON、独立 Markdown 报告导出和本地核心持久化 API；后续需要把前端保存动作接入该 API，并在审计页展示 AI 评审历史。
- 组合、风控、执行还不够像交易平台。
