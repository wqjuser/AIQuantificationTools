...
- Settings 平台状态已经把三类正式行情 adapter 收敛成 `marketDataAdapters` 只读契约：AKShare、yfinance 和 ccxt 会暴露 adapter 名称、provider、公开 OHLCV route、能力、周期覆盖、是否需要 API/交易密钥、cache scope、安全说明和按 adapter market 聚合的缓存诊断；前端 API client 会把该字段作为必需契约校验，并在 Settings 页面展示 fresh/stale/empty、缓存上下文数和缓存行数，避免真实行情接入状态只停留在后端实现细节里。该能力仍不读取密钥值、不连接券商、不解锁实盘执行。
