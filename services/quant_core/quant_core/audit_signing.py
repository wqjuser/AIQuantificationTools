...
@dataclass(frozen=True)
class AuditSigningKeyEnvironmentBinding:
    binding_id: str
    materialization_id: str
    plan_event_id: str
    current_active_key_id: str
...
@dataclass(frozen=True)
class AuditSigningKeyRuntimeReloadPlan:
    plan_id: str
    binding_id: str
    materialization_id: str
    plan_event_id: str
...
@dataclass(frozen=True)
class AuditSigningKeyRuntimeReloadExecution:
    execution_id: str
    plan_id: str
    binding_id: str
    materialization_id: str
...
def build_audit_signing_key_environment_binding(
    materialization: dict[str, Any],
    *,
    binding_mode: str = "",
    confirmations: dict[str, Any] | None = None,
    ...
    if not isinstance(materialization, dict):
        raise ValueError("audit_signing_key_secret_materialization_required")
    if not isinstance(confirmations, dict):
        confirmations = {}

    materialization_id = str(materialization.get("materializationId") or "").strip()
    plan_event_id = str(materialization.get("planEventId") or "").strip()
    current_key_id = str(materialization.get("currentActiveKeyId") or "").strip()
    current_fingerprint = str(materialization.get("currentActiveKeyFingerprint") or "").strip()
    proposed_key_id = str(materialization.get("proposedActiveKeyId") or "").strip()
    proposed_signer = str(materialization.get("proposedSigner") or "").strip()
    proposed_chain_id = str(materialization.get("proposedChainId") or "").strip()
    ...
    if not materialization_id:
        raise ValueError("audit_signing_key_environment_binding_materialization_id_required")
    if not plan_event_id:
        raise ValueError("audit_signing_key_environment_binding_plan_event_id_required")
    if not current_key_id:
        raise ValueError("audit_signing_key_environment_binding_current_key_required")
    if not current_fingerprint:
        raise ValueError("audit_signing_key_environment_binding_current_fingerprint_required")
    if not proposed_key_id:
        raise ValueError("audit_signing_key_environment_binding_proposed_key_required")
    if not backend:
        raise ValueError("audit_signing_key_environment_binding_backend_required")
    if not manifest_path:
        raise ValueError("audit_signing_key_environment_binding_manifest_path_required")
    if not normalized_binding_mode:
        raise ValueError("audit_signing_key_environment_binding_mode_required")
    if not required_env_vars:
        raise ValueError("audit_signing_key_environment_binding_required_env_vars_required")

    blocked_reasons: list[str] = []
    required_confirmations: list[dict[str, str]] = []
    for confirmation_id, payload_key, label, blocked_reason in _audit_signing_key_environment_binding_confirmation_specs():
        confirmed = _payload_bool(confirmations, payload_key)
        required_confirmations.append(
            {
                "id": confirmation_id,
    ...
    unique_blocked_reasons = list(dict.fromkeys(blocked_reasons))
    normalized_operator = str(operator or "local-operator").strip() or "local-operator"
    return AuditSigningKeyEnvironmentBinding(
        binding_id=str(binding_id or f"audit-signing-key-environment-binding-{proposed_key_id}-{uuid4()}"),
        materialization_id=materialization_id,
...
def audit_signing_key_environment_binding_to_payload(
    materialization: dict[str, Any],
    *,
    binding_mode: str = "",
    confirmations: dict[str, Any] | None = None,
    ...
    result = build_audit_signing_key_environment_binding(
        materialization,
    ...
    return _audit_signing_key_environment_binding_payload(result)


def audit_signing_key_environment_binding_payload_from_audit_event(event: Any) -> dict[str, Any] | None:
    if getattr(event, "event_type", "") != "audit_signing_key_environment_binding":
        return None
    metadata = getattr(event, "metadata", {})
    if not isinstance(metadata, dict):
        return None
    ...
    required_confirmations: list[dict[str, str]] = []
    for confirmation_id, _payload_key, label, _blocked_reason in _audit_signing_key_environment_binding_confirmation_specs():
        if required_ids and confirmation_id not in required_ids:
            continue
        required_confirmations.append(
            {
...
def audit_signing_key_environment_binding_to_audit_event_payload(binding: dict[str, Any]) -> dict[str, Any]:
    binding_id = _payload_text(binding, "bindingId")
    if not binding_id:
        raise ValueError("audit_signing_key_environment_binding_id_required")
    created_at = _parse_report_generated_at(_payload_text(binding, "recordedAt"))
    required_confirmations = (
        binding.get("requiredConfirmations") if isinstance(binding.get("requiredConfirmations"), list) else []
    )
    required_ids = [
        _payload_text(item, "id")
        for item in required_confirmations
    ...
    proposed_key_id = _payload_text(binding, "proposedActiveKeyId")
    return {
        "schemaVersion": 1,
        "eventId": binding_id,
        ...
        "source": "audit-signing-key-ledger",
        "summary": f"Audit signing key environment binding recorded for {proposed_key_id}.",
        ...
        "metadata": _redact_sensitive_fields(
            {
                "bindingId": binding_id,
                "materializationId": _payload_text(binding, "materializationId"),
                "planEventId": _payload_text(binding, "planEventId"),
                "currentActiveKeyId": _payload_text(binding, "currentActiveKeyId"),
                "currentActiveKeyFingerprint": _payload_text(binding, "currentActiveKeyFingerprint"),
                "proposedActiveKeyId": proposed_key_id,
                "proposedSigner": _payload_text(binding, "proposedSigner"),
                "proposedChainId": _payload_text(binding, "proposedChainId"),
                "status": _payload_text(binding, "status") or "blocked",
                "operator": _payload_text(binding, "operator") or "local-operator",
                "bindingMode": _payload_text(binding, "bindingMode") or "container_env_reference",
                "backend": _payload_text(binding, "backend"),
                "manifestPath": _payload_text(binding, "manifestPath"),
                "requiredEnvVars": _payload_string_list(binding.get("requiredEnvVars")),
                "blockedReasons": _payload_string_list(binding.get("blockedReasons")),
                "requiredConfirmationIds": required_ids,
                "confirmedConfirmationIds": confirmed_ids,
...
def _audit_signing_key_environment_binding_payload(result: AuditSigningKeyEnvironmentBinding) -> dict[str, Any]:
    return {
        "schemaVersion": 1,
...
def _audit_signing_key_environment_binding_confirmation_specs() -> list[tuple[str, str, str, str]]:
    return [
        (
...
def build_audit_signing_key_runtime_reload_plan(
    ...
    *,
    ...
    if not isinstance(environment_binding, dict):
        raise ValueError("audit_signing_key_environment_binding_required")
    if not isinstance(confirmations, dict):
        confirmations = {}
    ...
    plan_event_id = str(environment_binding.get("planEventId") or "").strip()
    current_key_id = str(environment_binding.get("currentActiveKeyId") or "").strip()
    current_fingerprint = str(environment_binding.get("currentActiveKeyFingerprint") or "").strip()
    proposed_key_id = str(environment_binding.get("proposedActiveKeyId") or "").strip()
    proposed_signer = str(environment_binding.get("proposedSigner") or "").strip()
    proposed_chain_id = str(environment_binding.get("proposedChainId") or "").strip()
    ...
    backend = str(environment_binding.get("backend") or "").strip()
    ...
    if not binding_id:
        raise ValueError("audit_signing_key_runtime_reload_plan_binding_id_required")
    if not materialization_id:
        raise ValueError("audit_signing_key_runtime_reload_plan_materialization_id_required")
    if not plan_event_id:
        raise ValueError("audit_signing_key_runtime_reload_plan_plan_event_id_required")
    if not current_key_id:
        raise ValueError("audit_signing_key_runtime_reload_plan_current_key_required")
    if not current_fingerprint:
        raise ValueError("audit_signing_key_runtime_reload_plan_current_fingerprint_required")
    if not proposed_key_id:
        raise ValueError("audit_signing_key_runtime_reload_plan_proposed_key_required")
    if not normalized_reload_mode:
        raise ValueError("audit_signing_key_runtime_reload_plan_mode_required")
    if not normalized_window_id:
        raise ValueError("audit_signing_key_runtime_reload_plan_window_required")
    if not binding_mode:
        raise ValueError("audit_signing_key_runtime_reload_plan_binding_mode_required")
    if not backend:
        raise ValueError("audit_signing_key_runtime_reload_plan_backend_required")
    if not manifest_path:
        raise ValueError("audit_signing_key_runtime_reload_plan_manifest_path_required")
    if not required_env_vars:
        raise ValueError("audit_signing_key_runtime_reload_plan_required_env_vars_required")

    blocked_reasons: list[str] = []
    required_confirmations: list[dict[str, str]] = []
    for confirmation_id, payload_key, label, blocked_reason in _audit_signing_key_runtime_reload_plan_confirmation_specs():
        confirmed = _payload_bool(confirmations, payload_key)
        required_confirmations.append(
            {
                "id": confirmation_id,
    ...
    unique_blocked_reasons = list(dict.fromkeys(blocked_reasons))
    normalized_operator = str(operator or "local-operator").strip() or "local-operator"
    return AuditSigningKeyRuntimeReloadPlan(
        plan_id=str(plan_id or f"audit-signing-key-runtime-reload-plan-{proposed_key_id}-{uuid4()}"),
        binding_id=binding_id,
        materialization_id=materialization_id,
        plan_event_id=plan_event_id,
        current_active_key_id=current_key_id,
...
def audit_signing_key_runtime_reload_plan_to_payload(
    environment_binding: dict[str, Any],
    *,
    reload_mode: str = "",
    maintenance_window_id: str = "",
    ...
    result = build_audit_signing_key_runtime_reload_plan(
        environment_binding,
    ...
    return _audit_signing_key_runtime_reload_plan_payload(result)


def audit_signing_key_runtime_reload_plan_payload_from_audit_event(event: Any) -> dict[str, Any] | None:
    if getattr(event, "event_type", "") != "audit_signing_key_runtime_reload_plan":
        return None
    metadata = getattr(event, "metadata", {})
    if not isinstance(metadata, dict):
        return None
    ...
    required_confirmations: list[dict[str, str]] = []
    for confirmation_id, _payload_key, label, _blocked_reason in _audit_signing_key_runtime_reload_plan_confirmation_specs():
        if required_ids and confirmation_id not in required_ids:
            continue
        required_confirmations.append(
            {
...
def audit_signing_key_runtime_reload_plan_to_audit_event_payload(plan: dict[str, Any]) -> dict[str, Any]:
    plan_id = _payload_text(plan, "planId")
    if not plan_id:
        raise ValueError("audit_signing_key_runtime_reload_plan_id_required")
    created_at = _parse_report_generated_at(_payload_text(plan, "recordedAt"))
    required_confirmations = (
        plan.get("requiredConfirmations") if isinstance(plan.get("requiredConfirmations"), list) else []
    )
    required_ids = [
        _payload_text(item, "id")
        for item in required_confirmations
    ...
    proposed_key_id = _payload_text(plan, "proposedActiveKeyId")
    status = _payload_text(plan, "status") or "blocked"
    ...
    return {
        ...
        "source": "audit-signing-key-ledger",
        "summary": f"Audit signing key runtime reload plan {status_label} for {proposed_key_id}.",
        "detail": "Runtime reload plan records orchestration evidence only; no restart is executed and live trading remains blocked.",
        "metadata": _redact_sensitive_fields(
            {
                "planId": plan_id,
                "bindingId": _payload_text(plan, "bindingId"),
                "materializationId": _payload_text(plan, "materializationId"),
                "planEventId": _payload_text(plan, "planEventId"),
                "currentActiveKeyId": _payload_text(plan, "currentActiveKeyId"),
                "currentActiveKeyFingerprint": _payload_text(plan, "currentActiveKeyFingerprint"),
                "proposedActiveKeyId": proposed_key_id,
                "proposedSigner": _payload_text(plan, "proposedSigner"),
                "proposedChainId": _payload_text(plan, "proposedChainId"),
                "status": status,
                "operator": _payload_text(plan, "operator") or "local-operator",
                "reloadMode": _payload_text(plan, "reloadMode") or "manual_container_reload_plan",
                "maintenanceWindowId": _payload_text(plan, "maintenanceWindowId"),
                "bindingMode": _payload_text(plan, "bindingMode") or "container_env_reference",
                "backend": _payload_text(plan, "backend"),
                "manifestPath": _payload_text(plan, "manifestPath"),
                "requiredEnvVars": _payload_string_list(plan.get("requiredEnvVars")),
                "blockedReasons": _payload_string_list(plan.get("blockedReasons")),
                "requiredConfirmationIds": required_ids,
                "confirmedConfirmationIds": confirmed_ids,
...
def _audit_signing_key_runtime_reload_plan_payload(result: AuditSigningKeyRuntimeReloadPlan) -> dict[str, Any]:
    return {
        "schemaVersion": 1,
...
def _audit_signing_key_runtime_reload_plan_confirmation_specs() -> list[tuple[str, str, str, str]]:
    return [
        (
...
def build_audit_signing_key_runtime_reload_execution(
    ...
    *,
    ...
    if not isinstance(runtime_reload_plan, dict):
        raise ValueError("audit_signing_key_runtime_reload_plan_required")
    if not isinstance(confirmations, dict):
        confirmations = {}
    ...
    plan_event_id = str(runtime_reload_plan.get("planEventId") or "").strip()
    current_key_id = str(runtime_reload_plan.get("currentActiveKeyId") or "").strip()
    current_fingerprint = str(runtime_reload_plan.get("currentActiveKeyFingerprint") or "").strip()
    proposed_key_id = str(runtime_reload_plan.get("proposedActiveKeyId") or "").strip()
    proposed_signer = str(runtime_reload_plan.get("proposedSigner") or "").strip()
    proposed_chain_id = str(runtime_reload_plan.get("proposedChainId") or "").strip()
    normalized_execution_mode = str(execution_mode or "manual_controlled_reload_evidence").strip()
    ...
    backend = str(runtime_reload_plan.get("backend") or "").strip()
    ...
    if not plan_id:
        raise ValueError("audit_signing_key_runtime_reload_execution_plan_id_required")
    if not binding_id:
        raise ValueError("audit_signing_key_runtime_reload_execution_binding_id_required")
    if not materialization_id:
        raise ValueError("audit_signing_key_runtime_reload_execution_materialization_id_required")
    if not plan_event_id:
        raise ValueError("audit_signing_key_runtime_reload_execution_plan_event_id_required")
    if not current_key_id:
        raise ValueError("audit_signing_key_runtime_reload_execution_current_key_required")
    if not current_fingerprint:
        raise ValueError("audit_signing_key_runtime_reload_execution_current_fingerprint_required")
    if not proposed_key_id:
        raise ValueError("audit_signing_key_runtime_reload_execution_proposed_key_required")
    if not normalized_execution_mode:
        raise ValueError("audit_signing_key_runtime_reload_execution_mode_required")
    if not reload_mode:
        raise ValueError("audit_signing_key_runtime_reload_execution_reload_mode_required")
    if not maintenance_window_id:
        raise ValueError("audit_signing_key_runtime_reload_execution_window_required")
    if not binding_mode:
        raise ValueError("audit_signing_key_runtime_reload_execution_binding_mode_required")
    if not backend:
        raise ValueError("audit_signing_key_runtime_reload_execution_backend_required")
    if not manifest_path:
        raise ValueError("audit_signing_key_runtime_reload_execution_manifest_path_required")
    if not required_env_vars:
        raise ValueError("audit_signing_key_runtime_reload_execution_required_env_vars_required")

    blocked_reasons: list[str] = []
    required_confirmations: list[dict[str, str]] = []
    for confirmation_id, payload_key, label, blocked_reason in _audit_signing_key_runtime_reload_execution_confirmation_specs():
        confirmed = _payload_bool(confirmations, payload_key)
        required_confirmations.append(
            {
                "id": confirmation_id,
    ...
    unique_blocked_reasons = list(dict.fromkeys(blocked_reasons))
    normalized_operator = str(operator or "local-operator").strip() or "local-operator"
    return AuditSigningKeyRuntimeReloadExecution(
        execution_id=str(execution_id or f"audit-signing-key-runtime-reload-execution-{proposed_key_id}-{uuid4()}"),
        plan_id=plan_id,
        binding_id=binding_id,
        materialization_id=materialization_id,
        plan_event_id=plan_event_id,
...
def audit_signing_key_runtime_reload_execution_to_payload(
    runtime_reload_plan: dict[str, Any],
    *,
    execution_mode: str = "",
    confirmations: dict[str, Any] | None = None,
    ...
    result = build_audit_signing_key_runtime_reload_execution(
        runtime_reload_plan,
    ...
    return _audit_signing_key_runtime_reload_execution_payload(result)


def audit_signing_key_runtime_reload_execution_payload_from_audit_event(event: Any) -> dict[str, Any] | None:
    if getattr(event, "event_type", "") != "audit_signing_key_runtime_reload_execution":
        return None
    metadata = getattr(event, "metadata", {})
    if not isinstance(metadata, dict):
        return None
    ...
    required_confirmations: list[dict[str, str]] = []
    for confirmation_id, _payload_key, label, _blocked_reason in _audit_signing_key_runtime_reload_execution_confirmation_specs():
        if required_ids and confirmation_id not in required_ids:
            continue
        required_confirmations.append(
            {
...
def audit_signing_key_runtime_reload_execution_to_audit_event_payload(execution: dict[str, Any]) -> dict[str, Any]:
    execution_id = _payload_text(execution, "executionId")
    if not execution_id:
        raise ValueError("audit_signing_key_runtime_reload_execution_id_required")
    created_at = _parse_report_generated_at(_payload_text(execution, "recordedAt"))
    required_confirmations = (
        execution.get("requiredConfirmations") if isinstance(execution.get("requiredConfirmations"), list) else []
    )
    required_ids = [
        _payload_text(item, "id")
        for item in required_confirmations
    ...
    proposed_key_id = _payload_text(execution, "proposedActiveKeyId")
    status = _payload_text(execution, "status") or "blocked"
    status_label = "blocked" if status == "blocked" else "recorded"
    return {
        "schemaVersion": 1,
        ...
        "source": "audit-signing-key-ledger",
        "summary": f"Audit signing key runtime reload execution {status_label} for {proposed_key_id}.",
        "detail": "Runtime reload execution stores operator evidence only; no restart is executed by this API and live trading remains blocked.",
        "metadata": _redact_sensitive_fields(
            {
                "executionId": execution_id,
                "planId": _payload_text(execution, "planId"),
                "bindingId": _payload_text(execution, "bindingId"),
                "materializationId": _payload_text(execution, "materializationId"),
                "planEventId": _payload_text(execution, "planEventId"),
                "currentActiveKeyId": _payload_text(execution, "currentActiveKeyId"),
                "currentActiveKeyFingerprint": _payload_text(execution, "currentActiveKeyFingerprint"),
                "proposedActiveKeyId": proposed_key_id,
                "proposedSigner": _payload_text(execution, "proposedSigner"),
                "proposedChainId": _payload_text(execution, "proposedChainId"),
                "status": status,
                "operator": _payload_text(execution, "operator") or "local-operator",
                "recordedAt": created_at.isoformat(),
                "executionMode": _payload_text(execution, "executionMode") or "manual_controlled_reload_evidence",
                "reloadMode": _payload_text(execution, "reloadMode") or "manual_container_reload_plan",
                "maintenanceWindowId": _payload_text(execution, "maintenanceWindowId"),
                "bindingMode": _payload_text(execution, "bindingMode") or "container_env_reference",
                "backend": _payload_text(execution, "backend"),
                "manifestPath": _payload_text(execution, "manifestPath"),
                "requiredEnvVars": _payload_string_list(execution.get("requiredEnvVars")),
                "blockedReasons": _payload_string_list(execution.get("blockedReasons")),
                "requiredConfirmationIds": required_ids,
                "confirmedConfirmationIds": confirmed_ids,
...
def _audit_signing_key_runtime_reload_execution_payload(
    result: AuditSigningKeyRuntimeReloadExecution,
) -> dict[str, Any]:
    return {
        "schemaVersion": 1,
...
def _audit_signing_key_runtime_reload_execution_confirmation_specs() -> list[tuple[str, str, str, str]]:
    return [
        (
