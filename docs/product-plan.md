...
- CoinGecko Binance 交易对映射在分页或市场事实源限流时保留已验证结果，同一进程内的后续显式选股从未完成页继续；可能跨页的边界交易对与其余未覆盖候选保持未解析或市场事实缺失。续扫页若回退到旧排序锚点之前，则废弃当前映射并等待下一次显式请求从第一页重核。不再以通用 `502` 丢失已取得的精确映射，也不使用同名推断补齐覆盖；没有后台重试器或新持久化状态。
