...
连接能力默认关闭。只有管理员用版本库内的确定性迁移工具把现有 `aiqt` realm 收敛到同一 Client Profile、Client Policy、scope 与 `aiqt-mcp` 配置，并通过只读检查，才可在受控维护窗口设置 `AIQT_CLAUDE_CONNECT_ENABLED=true` 进行真实协议验收；验收失败必须立即关闭，验收通过后才可对普通用户发布。未设置、迁移漂移或验收失败时，认证会话投影必须让侧栏入口和安装动作保持不可用；不能只凭 HTTPS Origin 推断授权服务已经就绪。

自托管 Keycloak 固定启用 CIMD feature。Client Policy 只接受 `https` 且 metadata host 为 `claude.ai` 的 URL 型 client ID；metadata 只能描述 public client，并继续强制 Authorization Code、PKCE S256、用户同意、禁用 Implicit/ROPC、关闭 full scope。Claude hosted HTTPS callback 与 Claude Code loopback callback 必须来自受信任 metadata 并逐项实机验证。`aiqt:research:read` 仍只产生 canonical `/mcp` audience；Caddy 继续拒绝整个 anonymous client-registration 路径。
...
- 迁移工具只把版本库冻结的 CIMD、`aiqt-mcp`、realm default scopes，以及 `basic`/`aiqt:research:read` scope 与 mapper 字段写入内网 Keycloak Admin API；Audience mapper 必须精确指向 canonical `/mcp` resource，缺失、额外或漂移均失败关闭。重复执行幂等；管理员密码只通过交互提示进入进程，不写入参数、环境、日志或 Git。
