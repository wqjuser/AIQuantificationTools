...
## 当前阶段：Stage 4 组合与模拟交易（2026-07-11）
...
Stage 4 的组合模拟黄金路径实现和运行时验收已经交付并成为当前阶段：Portfolio 已把双 run 组合构建、确定性风控、人工审批、批量模拟成交、状态历史和账户回放收口为五步唯一入口，并可通过 `POST /api/portfolio/workflows` 写入 `stage4_portfolio_workflow`、通过 `GET /api/portfolio/workflows?baseRunId=...` 恢复权威快照。研究包用 `artifactCounts.stage4PortfolioWorkflows` 严格记录并回读该证据；`npm run docker:smoke:stage4 -- --no-build` 生成 `aiqt.stage4PortfolioPaperAcceptance`，`npm run docker:smoke:stage4:validate` 离线复核工作流/组合 hash、风险证据、审批与成交顺序、幂等重试、精确 replay、导出/导入/再回读和五项安全字段。真实浏览器验收已覆盖拒绝证据、刷新恢复、Audit 包/hash 回读和 375px 布局。Stage 3 保留为维护回归门禁，Stage 5 仍为后续规划。
...
- 阶段 4 · 组合与模拟交易：当前唯一交付阶段，黄金路径与运行时、Docker、桌面发布门禁已通过。
- 阶段 5 · 实盘准备：后续规划。
