...
- RED: `terminal-workbench.test.ts -t "searchable audit ledger"` failed because import audit aggregation had no `verifiedReportSignatureBuckets`; `layout-css.test.js -t "import audit events"` failed because the Audit panel did not render local-core verification buckets.
- GREEN: added `verifiedReportSignatureBuckets` grouped by verified/invalid status with latest run/file/exportPath/reason/source context, rendered them as positive/risk cards in the Audit import ledger, and kept single-event verification details on confirmed rows.
- DOCS: updated product plan, architecture notes, and this implementation log; next slice can start external certificate-chain support or fold verified import report evidence into generated Audit Markdown.
- RED: `terminal-workbench.test.ts -t "copyable audit evidence summary"` failed because Audit evidence summaries omitted local-core import report verification; `layout-css.test.js -t "export package browser"` failed because App did not pass import audit events into the summary; `terminal-api.test.ts -t "audit evidence summary metadata"` failed until JSON auditEvidenceSummary preserved importVerification.
- GREEN: `buildAuditEvidenceSummary` now consumes import audit events, derives verified/invalid local-core report verification buckets, includes them in copyText and Markdown, App passes the current import audit page, and JSON auditEvidenceSummary stores optional `importVerification` while accepting older summaries without it.
- DOCS: updated product plan, architecture notes, and this implementation log; next slice can use these verification buckets in signed Audit report events or begin external certificate-chain support.
- RED: `terminal-api.test.ts -t "ledger audit event when an audit evidence report"` failed because generated Audit report events did not persist importVerification metadata; `terminal-workbench.test.ts -t "audit report ledger from persisted report events"` failed because report ledger rows could not show/search local-core verification; `layout-css.test.js -t "audit evidence report history"` failed because the UI row did not render the compact import verification count.
- GREEN: `buildAuditEvidenceReportAuditEvent` now writes verified/invalid import verification counts plus latest status/source/exportPath/reason into `audit_evidence_report.metadata`; report ledger rows derive searchable `importVerificationDetail`, and the Audit report history row shows compact import verification counts.
- DOCS: updated product plan, architecture notes, and this implementation log; next slice can move toward external certificate-chain support or deeper report-signing policy around imported evidence.
- RED: `terminal-workbench.test.ts -t "audit report ledger from persisted report events"` failed because report ledger summaries did not aggregate local-core import verification counts; `layout-css.test.js -t "audit evidence report history"` failed because the report ledger toolbar did not show the summary fields.
- GREEN: `buildAuditEvidenceReportLedgerSummary` now totals import verification verified/invalid counts across report rows, and the Audit report history toolbar renders the compact import verification summary.
- DOCS: updated product plan, architecture notes, and this implementation log; next slice can continue toward external certificate-chain support or policy checks for signing reports that carry invalid imported evidence.
- RED: `test_audit_report_sign_api_rejects_invalid_import_verification_evidence` failed because reports carrying invalid local-core import verification could still be signed; `layout-css.test.js -t "audit evidence report history"` failed because the Sign button was not gated by import verification risk.
- GREEN: signing now rejects `audit_evidence_report` events with `importVerificationInvalid > 0` using `audit_report_import_verification_invalid`, and the Audit report ledger disables Sign with a visible import-verification policy reason.
- DOCS: updated product plan, architecture notes, and this implementation log; next slice can continue toward external certificate-chain support or signed package policy exports.
...
- RED: `terminal-workbench.test.ts -t "copyable audit evidence summary"` failed because Audit Evidence Summary did not export blocked import policy evidence, and `terminal-api.test.ts -t "audit evidence summary metadata"` lacked a JSON schema slot for it.
- GREEN: `buildAuditEvidenceSummary` now derives `importPolicyBlockerBuckets` from blocked import audit evidence, copyText and Markdown include an `Import Policy Blockers` section, and `aiqt.auditEvidenceSummary.importPolicyBlockers` preserves the same machine-readable buckets while older packages remain valid.
- DOCS: updated product plan, architecture notes, and this implementation log; next slice can continue toward external certificate-chain support or broader cross-symbol backtest evidence.
- RED: `terminal-workbench.test.ts -t "cross-symbol"` failed because Backtest reports had no cross-symbol comparison model and exported Markdown lacked a `Cross-Symbol Comparison` evidence section.
- GREEN: added `buildBacktestCrossSymbolComparisonRows`, summary and filter helpers for same-market/timeframe audited runs, and appended a `Cross-Symbol Comparison` section to Backtest Markdown without changing strategy drafts or implying portfolio advice.
- DOCS: updated product plan, architecture notes, and this implementation log; next slice can move toward UI surfacing for cross-symbol evidence or deeper multi-symbol/portfolio backtesting.
- RED: `test_portfolio_backtest_combines_symbol_runs_with_weights_and_cash_buffer` failed because `quant_core.portfolio_backtest` did not exist; `test_portfolio_backtest_api_combines_audited_runs_by_weight` failed with HTTP 404 because `/api/portfolio/backtest` was not routed.
- GREEN: added `PortfolioBacktestEngine`, static target-weight `PortfolioLeg` aggregation, portfolio metrics/equity/contribution payloads, and `POST /api/portfolio/backtest` to combine already-audited run ids without rerunning research or emitting trade advice.
- DOCS: updated product plan and architecture notes; next slice should surface portfolio backtest evidence in the Portfolio workspace and then add rebalancing, exposure, and portfolio-risk diagnostics.
- RED: `terminal-api.test.ts -t "portfolio backtest"` failed because the web API client had no `/api/portfolio/backtest` URL builder or response contract; `terminal-workbench.test.ts -t "portfolio backtest draft"` failed because the Portfolio workspace model had no way to derive a cross-symbol basket from audited run history.
- GREEN: added the `runPortfolioBacktest` frontend client, `buildPortfolioBacktestDraft` model, and Portfolio workspace UI that generates a same-market/timeframe static-weight basket from audited runs, posts it to the core, and displays combination metrics, cash weight, equity point count, and leg contribution without creating trade advice.
- DOCS: updated product plan, architecture notes, and this implementation log; next slice should add portfolio rebalancing assumptions, exposure/correlation diagnostics, and portfolio-level report export.
...
- RED: `terminal-api.test.ts -t "portfolio report audit event"` failed because Portfolio Markdown exports had no `portfolio_report` event builder or current-run anchoring policy.
- GREEN: added `buildPortfolioBacktestReportAuditEvent`, stable Portfolio report event ids based on anchored run id and Markdown SHA-256, metadata for portfolio name, market/timeframe, cash, legs, diagnostics, data-quality boundary, and wired Portfolio Markdown export through `saveAuditEvent`.
- DOCS: updated product plan and architecture notes; next slice should decide whether `portfolio_report` joins Audit report history/signing and research package signature merging.
- RED: `terminal-workbench.test.ts -t "portfolio markdown report events"` failed because report ledger rows ignored `portfolio_report`; `layout-css.test.js -t "audit evidence report history"` failed because Audit only queried two report types; `test_portfolio_report_sign_and_verify_api_updates_signature_metadata` failed because the report signer rejected Portfolio report events.
- GREEN: added `portfolio_report` to Audit report ledger rows, exposed Portfolio report context/leg/equity counts in searchable rows, queried `audit_evidence_report,backtest_report,portfolio_report` from Audit, and allowed the local HMAC signer to sign/verify/revoke `aiqt.portfolioReport` events.
- DOCS: updated product plan and architecture notes; next slice can add external package handling if Portfolio reports become a first-class cross-run artifact, otherwise continue with portfolio rebalancing/exposure diagnostics.
...
- RED: `test_portfolio_backtest_outputs_pairwise_correlation_evidence` failed because `PortfolioBacktestRun` had no `correlation_pairs`; `terminal-workbench.test.ts terminal-api.test.ts -t "portfolio"` failed because Portfolio diagnostics and report metadata had no correlation risk row.
- GREEN: added backend pairwise Pearson correlation evidence from aligned leg equity returns, emitted `correlationPairs` in the portfolio API payload, accepted it in the frontend API contract, and surfaced the highest absolute pair as a Portfolio correlation risk diagnostic and Markdown report row.
- DOCS: updated product plan and architecture notes to distinguish current pairwise correlation review from future full covariance/risk decomposition.
- RED: `test_portfolio_backtest_outputs_static_allocation_events` failed because `PortfolioLeg` could not carry source run ids and `PortfolioBacktestRun` had no static allocation ledger; `terminal-workbench.test.ts -t "portfolio"` failed because Portfolio Markdown reports did not render allocation events.
- GREEN: added backend `allocationEvents` for audited leg allocations and cash buffer, preserved source run ids in `/api/portfolio/backtest`, accepted the payload in the web API contract, rendered the ledger in the Portfolio workspace and Markdown report, and recorded `allocationEventCount` on `portfolio_report` audit events without treating the rows as live orders.
- DOCS: updated product plan and architecture notes to record static allocation ledgers as current portfolio evidence while leaving true rebalance/order ledgers and covariance risk for future slices.
- RED: `test_portfolio_backtest_outputs_rebalance_review_events` failed because Portfolio runs had no end-weight rebalance review events; `terminal-workbench.test.ts -t "portfolio"` failed because Portfolio Markdown reports did not render a rebalance review ledger.
- GREEN: added backend `rebalanceEvents` for leg and cash ending-weight drift review, accepted the payload in the web API contract, rendered the review ledger in the Portfolio workspace and Markdown report, and recorded `rebalanceEventCount` on `portfolio_report` audit events without routing orders.
- DOCS: updated product plan and architecture notes to record rebalance review ledgers as current portfolio evidence while keeping true rebalance execution and live orders out of scope.
- RED: `test_portfolio_backtest_outputs_covariance_risk_summary` failed because Portfolio runs had no `covariance_risk`; `terminal-workbench.test.ts -t "portfolio"` failed because diagnostics and Markdown reports did not include covariance risk evidence.
- GREEN: added backend `covarianceRisk` from aligned leg equity returns using population covariance, accepted it in the web API contract, surfaced the largest covariance contribution as a Portfolio diagnostic, included a covariance risk section in Markdown reports, and recorded contribution count plus annualized volatility on `portfolio_report` audit metadata.
- DOCS: updated product plan and architecture notes to record covariance risk summary as current Portfolio evidence while keeping true rebalance execution, live orders, and fuller portfolio risk modeling out of scope.
- RED: `test_research_run_export_import_preserves_portfolio_paper_order_batches` and `test_research_run_export_import_preserves_portfolio_paper_order_history` failed because research run packages ignored persisted Portfolio paper order batches; `terminal-api.test.ts -t "rejects malformed portfolio paper order batches"` failed because the web export-package guard ignored malformed `portfolioPaperOrderBatches`.
- GREEN: added `portfolioPaperOrderBatches` to research run export/import, manifest counts, API export/import persistence, rollback/undo snapshots, web type guards, package browser rows, recent package index rows, and import diff count checks.
- DOCS: updated product plan and architecture notes to record Portfolio paper order batch portability while keeping live rebalance execution and broker routing out of scope.
- RED: `test_portfolio_paper_order_api_records_and_lists_batches` failed because portfolio paper order batches were stored without a shared audit event; `terminal-api.test.ts -t "records and loads portfolio paper order batches"` failed because the client dropped the returned audit event; `terminal-workbench.test.ts -t "portfolio paper order lifecycle"` failed because Execution had no model for portfolio order batch lifecycle rows.
- GREEN: added `portfolio_paper_order_batch` audit event generation on `/api/portfolio/paper-orders`, returned the event through the web API client, and surfaced portfolio paper order lifecycle rows in Execution/Portfolio using the deterministic audit event id and paper-only boundary.
- DOCS: updated product plan and architecture notes to record portfolio paper order audit event aggregation and execution-center lifecycle visibility.
- RED: `test_portfolio_paper_order_lifecycle_requires_operator_approval_before_simulation` failed because execution had no portfolio paper order state machine; `test_portfolio_paper_order_api_records_and_lists_batches` failed because `/api/portfolio/paper-orders` returned no lifecycle rows; `terminal-api.test.ts -t "records and loads portfolio paper order batches"` failed because the web client dropped lifecycle rows; `terminal-workbench.test.ts -t "portfolio paper order lifecycle"` failed because lifecycle rows had no execution-state summary.
- GREEN: added `build_portfolio_paper_order_lifecycle`, returned `portfolioPaperOrderLifecycle` from the paper-order batch API, recorded lifecycle state counts and routable order counts in the audit event metadata, accepted the lifecycle contract in the web client, and showed execution-state summaries in the Execution portfolio batch table.
- DOCS: updated product plan and architecture notes to record the portfolio paper order state machine while keeping real routing and fills out of scope until approval and simulation APIs exist.
- RED: `test_portfolio_paper_order_approval_api_records_review_and_updates_lifecycle` failed because portfolio paper orders had no persisted operator approval ledger; `terminal-api.test.ts -t "portfolio paper order approval"` failed because the web client had no approval URL or record/load helpers.
- GREEN: added `PortfolioPaperOrderApprovalStore`, `/api/portfolio/paper-order-approvals` POST/GET, approval audit events, lifecycle recomputation from stored approvals, and frontend approval client contracts that return approvals plus `portfolioPaperOrderLifecycle`.
- DOCS: updated product plan and architecture notes to record the paper-only operator approval ledger; next slice should add approval UI wiring and the simulated fill/order-state machine for `ready_for_simulation` orders.
- RED: `terminal-workbench.test.ts -t "portfolio paper order approval rows"` failed because the workbench had no single-order approval row model; `layout-css.test.js -t "portfolio paper order approvals"` failed because the app did not load approval lifecycle events or render approve/reject actions.
- GREEN: added lifecycle-aware portfolio paper order approval rows, loaded batch approval lifecycle after paper-order batches, refreshed lifecycle state after approve/reject, and rendered a compact operator approval queue inside Execution and Portfolio execution panels.
- DOCS: updated product plan and architecture notes to mark approval UI wiring complete; next slice is the paper-only simulated fill/order-state machine for `ready_for_simulation` orders.
- RED: `test_portfolio_paper_order_simulation_api_fills_ready_orders_and_records_audit_event` failed because there was no `PortfolioPaperOrderSimulationStore` or simulation route; `terminal-api.test.ts -t "portfolio paper order simulation"` failed because the web client had no simulation URL/helper; `layout-css.test.js -t "portfolio paper order simulations"` failed because App did not render simulation actions or receipts.
- GREEN: added a paper-only Portfolio paper order simulation store, `/api/portfolio/paper-order-simulations` POST/GET, simulation audit events, frontend record/load helpers, a "simulate fill" action for `ready_for_simulation` rows, and compact simulated-fill receipts in Execution/Portfolio.
- DOCS: updated product plan and architecture notes to record the first simulated-fill state machine; next slice should expand this into order-state history plus simulated account/position replay.
- RED: `test_research_run_export_import_preserves_portfolio_paper_order_history` failed because research run export packages only carried `portfolioPaperOrderBatches`; `terminal-api.test.ts -t "approvals and simulations in export packages"` failed because malformed `portfolioPaperOrderApprovals` / `portfolioPaperOrderSimulations` were accepted; `terminal-workbench.test.ts -t "artifact counts are unsafe"` and `-t "recent export package index"` failed because package inspection ignored the approval and simulated-fill ledgers.
- GREEN: added `portfolioPaperOrderApprovals` and `portfolioPaperOrderSimulations` to backend export/import payloads, manifest artifact counts, strict import validation, persistence rollback and undo snapshots; frontend package guards, package browser rows, recent package index rows and import diff checks now compare batches, approvals and fills separately.
- DOCS: updated product plan, architecture notes and this implementation log to mark Portfolio paper order approval/simulation portability complete while keeping real broker routing, order-state history and simulated account/position replay as the next execution-layer work.
- RED: `test_portfolio_paper_order_replay_rebuilds_cash_positions_and_orders` and `test_portfolio_paper_order_replay_api_returns_base_run_account_snapshot` failed because there was no replay helper or `/api/portfolio/paper-order-replay`; frontend API/model/layout tests failed because no client, workbench rows, or UI section consumed replay data.
- GREEN: added `build_portfolio_paper_order_replay`, `GET /api/portfolio/paper-order-replay`, frontend replay URL/load helpers, strict replay payload guards, workbench summary/position builders, and Execution/Portfolio UI wiring that refreshes paper account state after simulated fills.
- DOCS: updated product plan and architecture notes to mark portfolio simulated account/position replay complete while keeping real broker routing, order-state history, and adapter-state ledgers as upcoming execution-layer work.
- RED: `test_portfolio_paper_order_state_history_builds_order_timeline` failed because there was no state-history helper; `test_portfolio_paper_order_state_history_api_returns_batch_timeline` failed because `/api/portfolio/paper-order-state-history` returned 404; frontend API/model/layout tests failed because no client, row builder, or UI section consumed state-history data.
- GREEN: added `build_portfolio_paper_order_state_history`, `GET /api/portfolio/paper-order-state-history`, frontend URL/load helpers with strict payload guards, workbench timeline rows, and Execution/Portfolio UI wiring that refreshes compact order-state history after batch recording, operator review, and simulated fills.
- DOCS: updated product plan and architecture notes to mark portfolio paper order state history complete while keeping real broker routing, adapter-state ledgers, and fuller portfolio risk models as upcoming execution-layer work.
- RED: backend, frontend API, workbench, and layout tests failed because execution adapter state had no read-only ledger contract, URL, row model, or Settings audit rail.
- GREEN: added `build_execution_adapter_state_ledger`, `GET /api/execution/adapter-ledger`, frontend loader/type guards, compact ledger row builder, and Settings audit rail that shows paper-ready, config-required, and live-blocked adapter events without exposing secrets or unlocking live trading.
- DOCS: updated product plan and architecture notes to mark the pre-live execution adapter state ledger complete; next slice should persist adapter certification test runs, secret-store application status, controlled restart evidence, and broader portfolio risk models.
