...
- 研究运行审计库已经存在。
- 策略配置和回测合同已经存在。
- 回测实验室已经把审计 run 提升为 Backtest Report，集中展示指标、交易流水、权益曲线、诊断、同数据快照基准对比、证据包、AI 评审准备状态和执行交接状态。
- Backtest Report 已经支持导出 Markdown 审计报告，面向人工复核和留档；完整复现仍使用审计运行 JSON 导出包。
...
- 策略编辑仍然偏文本，不像生产级策略工坊。
- 数据源配置不是一等页面。
- 回测证据已经开始成为核心资产，后续还需要补参数扫描、多标的/组合回测和正式报告导出。
