...
- 组合级回测已经从后端能力进入 Portfolio 工作区：`PortfolioBacktestEngine` 可以把多个已审计单标的 `BacktestRun` 按目标权重聚合为组合权益曲线、现金缓冲、组合收益/回撤/交易数、数据质量和标的贡献；本地核心新增 `POST /api/portfolio/backtest`，输入已存在的审计 run id 与权重，输出组合级证据。前端 Portfolio 工作区会从同市场、同周期、带权益曲线的审计运行历史生成静态权重组合草稿，并可直接调用核心接口展示组合指标、现金权重、权益点数量和各标的贡献；当同市场 peer 不足时，Portfolio 工作区会基于当前自选列表生成对照审计计划，并可一键补跑最少需要的 peer 审计 run，让组合回测从 blocked 进入 ready。当前版本只做时间戳对齐的静态权重组合聚合，不做再平衡、组合交易流水、相关性分析或自动调仓。
