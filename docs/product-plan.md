...
Batch 4 的回放闸门已经接入晋级链路：`buildPreLiveReadinessChecklist` 现在会把 `paper-execution-replay` 作为第 6 个 gate，只有 replay gate 达到 `replay_ready` 才能继续成为 manual-route candidate；P2 pre-live acceptance manifest 也把 `paper-execution-replay` 设为 required check，缺少该检查的 manifest 会被标记为 invalid。
