...
- 黄金路径的行情数据步骤现在也沿用同一套缓存语义：fresh 缓存会显示“可支撑审计研究”，stale 缓存会把当前任务和审计清单的下一步原因写成“刷新行情后再运行审计研究”，并继续指向 Market 工作区的 `refresh-data` 动作。
...
- Stage 1 黄金路径现在会把行情缓存状态翻译成同样的任务语言：fresh 行情缓存通过 market-data 步骤并说明可支撑审计研究；stale 缓存会让当前任务卡、顶部 runbook 和 Audit runbook 都提示先刷新行情数据后再运行审计研究。
