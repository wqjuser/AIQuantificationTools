...
Stage 9 默认 Docker acceptance 不使用宿主生产凭据或真实生产网络，覆盖无凭据零制品、确定性 ready、规则/行情/价格/资金故障、连续性漂移、候选过期、真实 POST 幂等、revoke 网络前阻断、detached 阻断和 API 重启 hash 精确回读；可选真实只读门禁用实际 CCXT 方法守卫验证只读调用，只接受 ready 或纯资金不足安全阻断，并在重启后回读。2026-07-14 最新 accepted manifest hash 为 `32524b780c74b4971e55006c75ad0c7fb6e9167ed0e7aeec09ac00835ba7993b`。退出不要求生产账户入金；生产订单创建、查询、撤销、成交、转账、提现和 live route 仍不存在。
