...
- Audit 签名 Key 注册表面板继续补了“应用预检”阶段：用户必须显式确认新 secret 已在本地安全保存、当前 secret 已写入 legacy 注册表、操作员已复核 key/指纹/重启影响，前端才会调用 `/api/audit/signing-keys/rotation-apply`。后端只做只读安全预检，缺确认时返回 blocked，确认齐全时返回 ready_for_restart；结果会被压缩成 `audit_signing_key_rotation_apply` 事件写入审计账本，历史视图可按阻断原因、确认项 id、apply mode 和拟启用 key 搜索，仍不在 UI 或 API 中传输 raw secret，也不自动写环境变量或重启容器。本地核心现在还新增 `POST /api/audit/signing-keys/rotation-restart-evidence`：它必须引用已入账且 ready_for_restart 的 apply 事件，记录重启窗口执行、回滚计划、重启后验收和日志复核四项操作者确认，缺项返回 blocked，完整确认写入 `audit_signing_key_controlled_restart_evidence` 审计事件；payload 和账本 metadata 会递归脱敏 secret/token/apiKey/privateKey/password 字段，并继续固定 `liveTradingAllowed=false`。Audit UI 历史视图已经可以回读这些受控重启证据行，并按 operator、confirmation id、拟启用 key、模式和阻断原因搜索。下一步是补前端提交受控重启证据的操作入口，并继续推进真正的本地 secret-store 物化和重载编排。
