...
## 当前阶段：Stage 3 可审计 AI 评审（2026-07-11）

Stage 0 平台基础、Stage 1 行情研究与 Stage 2 策略实验现已进入维护状态。唯一当前阶段是 Stage 3 AI Review。

Stage 4 组合与模拟交易、Stage 5 实盘准备仍为后续规划。Stage 2 于 2026-07-11 完成阶段退出，其 canonical snapshot、holdout-safe selection、确定性 hash 与精确回放继续作为 Stage 3 回归门禁。
...
Stage 4 的组合模拟黄金路径实现和运行时验收已经交付，但阶段状态尚未切换：Portfolio 已把双 run 组合构建、确定性风控、人工审批、批量模拟成交、状态历史和账户回放收口为五步唯一入口，并可通过 `POST /api/portfolio/workflows` 写入 `stage4_portfolio_workflow`、通过 `GET /api/portfolio/workflows?baseRunId=...` 恢复权威快照。研究包用 `artifactCounts.stage4PortfolioWorkflows` 严格记录并回读该证据；`npm run docker:smoke:stage4 -- --no-build` 生成 `aiqt.stage4PortfolioPaperAcceptance`，`npm run docker:smoke:stage4:validate` 离线复核工作流/组合 hash、风险证据、审批与成交顺序、幂等重试、精确 replay、导出/导入/再回读和五项安全字段。真实浏览器验收已覆盖拒绝证据、刷新恢复、Audit 包/hash 回读和 375px 布局。Task 10 完整发布门禁通过前，Stage 3 仍是唯一当前阶段，Stage 4 仍为后续规划。
...
- 阶段 3 · AI 评审：当前唯一交付阶段，范围仅限可审计、Provider 可选、失败可降级的证据评审闭环。
- 阶段 4 · 组合与模拟交易：黄金路径与运行时验收已交付，Task 10 发布门禁通过前仍为后续规划。
- 阶段 5 · 实盘准备：后续规划。
...
仍暂缓新增功能、只允许修复阻断性 bug 的后续阶段是阶段 4 组合与模拟交易、阶段 5 实盘准备。
