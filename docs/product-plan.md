...
- Backtest Report 已经支持导出 Markdown 审计报告，面向人工复核和留档；报告内包含同一审计快照上的参数扫描摘要、SMA 参数敏感性表和同上下文运行对比矩阵，且当入场规则叠加 RSI 动量确认或成交量确认时会同步扫描 RSI 阈值与 VOL 窗口候选，非当前参数候选可以暂存回策略工坊并强制重新审计；同一份报告也会作为 `aiqt.backtestReport` 顶层 artifact 附加到研究运行 JSON 复现包，跨机器导入时保留 Markdown 内容、内容 SHA-256、运行上下文和非投资建议边界。
...
- 数据源配置不是一等页面。
- 回测证据已经开始成为核心资产，参数扫描已从单纯 SMA 窗口推进到 SMA+RSI 确认阈值和 SMA+VOL 窗口组合，并有当前排名、复审候选、风险行数摘要和同上下文运行对比矩阵；后续还需要补更正式的多参数扫描、多标的/组合回测和报告模板。
