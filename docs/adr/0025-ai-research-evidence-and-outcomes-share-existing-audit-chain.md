...
作为 M4 的上游权威证据，外部 AI 评审在用户逐次授权后发送服务端冻结快照中的原始已完成 K 线，并同时绑定源快照哈希、已完成子集哈希、区间和省略的形成中行数；形成中和未来 K 线不得进入外发证据。Provider 成功返回但响应包含执行语义时，本地安全校验仍失败关闭，只在评审审计中保存“响应已返回”、校验阶段、错误类别和字段路径，不保存被拒绝原文，也不覆盖本地确定性评估。
...
- 浏览器不能上传 K 线或外部响应事实；外发内容、完成状态和诊断均由服务端从受保护证据派生。
