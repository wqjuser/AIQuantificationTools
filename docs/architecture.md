...
- `cache.py`：SQLite 本地行情缓存，按 `market + symbol + timeframe + timestamp` 去重，并提供总行数、上下文数量和最新 K 线时间的只读统计。
...
- `settings.py`：只读平台设置状态契约，汇总数据源、可选本地 API Key 配置标志、SQLite 行情缓存路径/行数/上下文/最新时间和执行适配器安全状态；该模块不返回任何密钥值，也不解锁实盘交易。
...
- `GET /api/settings/status`：返回 Settings 工作区只读状态，包括 A 股/美股/加密数据源、可选 API Key 是否本地配置、SQLite 行情缓存路径、缓存行数、市场/标的/周期上下文数量、最新 K 线时间和执行适配器闸门；响应只暴露布尔配置状态，不包含 secret。
