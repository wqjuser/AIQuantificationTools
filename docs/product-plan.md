...
## 当前阶段：Stage 7 已退出，下一阶段待规划（2026-07-13）

Stage 0 至 Stage 7 已全部进入维护状态，当前不自动激活新的开发阶段。下一阶段需要单独完成路线设计、人工确认和安全验收；在此之前不创建、查询、撤销或同步生产订单，不访问成交、转账或提现接口，不开放真实资金委托和 live route。
...
阶段 0 至阶段 7 均只允许维护、回归门禁与阻断性缺陷修复。Stage 6 只保留已验收的 Binance Spot Testnet 委托与恢复能力；Stage 7 只保留已验收的生产只读准入能力。选择第二个 adapter、扩大生产账户读取范围、创建或管理生产订单、成交同步、转账、提现和 live route 均须进入后续独立设计、人工授权与验收。
