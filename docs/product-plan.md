...
- Settings 工作区已经开始读取本地核心 `/api/settings/status`，展示 A 股、美股、加密货币数据源状态、可选 API Key 是否本地配置、SQLite 行情缓存路径、缓存行数、市场/标的/周期上下文数量、最新 K 线时间、最近缓存上下文清单和执行适配器安全闸门；该接口只返回配置状态，不返回密钥值。
- 导出、导入和完整性校验已经存在。
- 中英文 i18n 基础已经存在。
