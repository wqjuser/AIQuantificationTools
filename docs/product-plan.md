...
- 研究运行审计库已经存在。
- 策略配置和回测合同已经存在。
- 回测实验室已经把审计 run 提升为 Backtest Report，集中展示指标、交易流水、权益曲线、诊断、证据包、AI 评审准备状态和执行交接状态。
- AI 评审已经开始绑定审计证据。
...
- 策略编辑仍然偏文本，不像生产级策略工坊。
- 数据源配置不是一等页面。
- 回测证据已经开始成为核心资产，后续还需要补基准对比、参数扫描和多标的/组合回测。
...
5. 扩展 Backtest Report，继续补基准对比、参数扫描、多标的回测和报告导出。
