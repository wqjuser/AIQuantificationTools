...
- 免费数据源：A 股 AKShare、美股 yfinance/Alpha Vantage 继续以适配器方式预留；加密货币 `CcxtMarketDataAdapter` 已支持公开 OHLCV 归一化，默认仍用 demo fallback 保证开箱可跑。
...
- 实盘图表：参考 QuantDinger 的 `/api/kline` + `klinecharts` 方法，前端图表从 `/api/market/klines` 拉取 OHLCV，A 股日线优先使用腾讯 `fqkline`，加密货币优先 Binance/Coinbase REST 并降级到正式 ccxt adapter。
