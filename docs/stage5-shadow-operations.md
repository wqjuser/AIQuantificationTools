...
完整 smoke 会先生成 Stage 3 deterministic baseline 和 5 个独立 Stage 4 权威 workflow，执行五类确定性 Shadow 演练，再为已对账的 workflow 绑定真实持久化 terminal adapter paper evidence，生成 Sandbox 准入决策；随后调用服务端 CCXT 只读健康端点，并通过既有 execution/review API 持久化默认无凭据的 blocked probe 链，验证这些真实源事件既不能生成 Sandbox 授权预检，也不能补造授权复核。七份上游证据全部通过既有 validator 后，链路生成 `data/stage5-exit-acceptance.json`，同步八份文件到 API 数据卷，并验证 latest API 回读。所有 validate 命令都只离线复核 manifest，不访问网络。

## CI 发布门禁

GitHub Actions 在已有 Docker 镜像上运行自包含的完整 Stage 5 smoke 和完整 validate。smoke 已内含 Stage 3 baseline、Stage 4 portfolio acceptance、全部 Stage 5 链路和顶层退出回读，并在结束时关闭容器；validate 随后只读取 JSON。CI 不注入交易所密钥。每次运行都会上传 `stage5-release-manifests`，其中包含：
...
顶层退出清单必须固定绑定七个源路径及其 SHA-256，状态为 `accepted_for_maintenance`，并保持 `authorizationEffective=false`、`sandboxOrderSubmissionAllowed=false` 和全部 live/order route 阻断。`GET /api/stage5/exit-acceptance/latest` 会重新计算源文件 hash；缺失或漂移返回 missing/invalid，不能继续显示历史通过。Execution 只回读该状态，不提供重新签发或解锁动作。
