...
候选绑定 workflow hash、Stage 6 authorization hash、Stage 8 continuity hash 和精确订单 hash。Stage 8 current continuity 先把查询时刻规范化为最新 probe/control 来源时间，避免普通轮询制造假漂移；任何 probe/control 或其他来源变化仍会改变 hash。任何来源缺失、导入为 detached、时间过期或内容漂移都直接阻断，不允许手填订单补链。
...
候选和复核写入现有 `AuditEventStore`，进入研究运行导出包，并由 manifest 分别计数。候选保留经既有 validator 校验且必须等于 canonical 重建结果的精确 Stage 8 continuity 快照与 hash；导入时重验 exact schema、SHA-256、事件绑定和 Stage 4/6/8 候选来源链，然后标记 detached。Audit 提供候选/复核专属只读行并标识 detached；导入副本不恢复到 Execution，页面静置到期也会自动撤下旧候选的复核入口。

退出验收覆盖无凭据零制品、确定性成功、规则漂移、陈旧报价、不利价格、资金不足、连续性漂移、候选过期、Stage 8 revoke 网络前阻断、真实 POST 重复请求、detached 阻断、API 重启精确回读和固定 live-blocked 边界。可选真实只读验收把 CCXT 方法守卫注入同一次权威 POST，证明只调用市场/报价/余额读取且 observation blocker 与 API blocker 一致，只允许 ready 或纯资金不足安全阻断，并在 API 重启后回读。退出不要求生产账户入金，也不使用生产交易凭据。
