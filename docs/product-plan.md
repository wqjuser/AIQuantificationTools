...
最新更新：Stage 1/P0 日常使用收口现在有了本地 CLI 报告。新增 `tools/stage1_daily_use.py`、`npm run stage1:daily` 和 `npm run stage1:daily:validate`，会读取已有 `data/p0-acceptance.json`、`data/p1-acceptance.json` 与 `data/desktop-release.json`，复用后端验收/readback 状态生成 `data/stage1-daily-use.json`，把“干净环境开箱”和“桌面发布”归并为 ready/review/blocked 两行。该命令只做本地证据聚合和离线校验，不自动运行 P0/P1/P2 smoke、不构建桌面包、不写审计事件、不连接券商、不提交订单，也不改变 `paperOnly=true`、`liveTradingAllowed=false`、`liveBlockedBoundary=true`。

最新更新：Stage 1/P0 日常使用报告已经从命令行进入本地核心 readback 和首页第一屏。报告契约迁移到 `quant_core.stage1_daily_use`，CLI 继续作为薄包装使用同一套校验器；新增 `GET /api/stage1/daily-use/latest`，前端新增 `loadStage1DailyUseLatest` 与 `buildStage1DailyUseSummary`，首页日常收口卡会读取 `data/stage1-daily-use.json` 并把干净环境/桌面发布两行显示为同源报告状态。该 readback 只读取本地报告文件并提供 missing/invalid/ready/review/blocked 状态，不自动生成报告、不运行 smoke、不打包桌面端、不写审计事件、不连接券商、不提交订单，也不改变 `liveTradingAllowed=false`。
