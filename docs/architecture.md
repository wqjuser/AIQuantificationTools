...
- `runs.py`：SQLite 研究运行审计记录，保存 run id、结构化策略配置、策略 revision、数据质量、数据行数、指标、交易流水、权益曲线、诊断和 AI 决策。
...
- `GET /api/research/runs?limit=5`：返回最近研究运行审计记录，用于终端 Run History 面板；每条记录包含 `strategyConfig` 和 `dataQuality`，分别记录结构化策略规则与数据源质量。
- `GET /api/research/runs/{runId}`：返回单次研究运行的完整审计详情，用于按 id 回放、导出、AI 报告和后续执行绑定；详情同样携带 `strategyConfig` 和 `dataQuality`，回放时优先用结构化策略生成策略快照，并把不完整或 fallback 数据显示为 warning。
