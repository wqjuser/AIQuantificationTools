...
- `POST /api/research/runs/import`：导入 `aiqt.researchRun.export` JSON 包，将其中的 `researchRun` 写入本地审计库，将 `researchRun.strategyConfig` 以原始 revision 还原为本地 audited 策略版本，将 `paperExecutions` 还原到本地 `PaperExecutionStore`，将 `aiReviewRuns[].record` 还原到本地 `AiReviewRunStore`，并返回完整 run detail；若导入包声称允许实盘交易、integrity hash 校验失败、manifest 与数据快照/交易流水/研究笔记/模拟执行/AI 评审记录数量不一致，则返回 `invalid_research_run_export`，导入不能绕过本地风控闸门。
