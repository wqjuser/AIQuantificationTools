...
- P0 当前任务卡现在会在可用性摘要下方显示“最近证据”结果条：优先展示最新 paper-only 模拟执行 id、委托数量和闸门通过数；如果尚未提交模拟委托，则展示 Golden Path 的 latestRunId 和当前状态；没有证据时提示先运行审计研究流水线。该结果条只用于把用户刚完成的动作和 Audit/Execution 工作区连接起来，不写后端状态、不创建订单、不改变 Golden Path 语义，也不放开实盘交易。
