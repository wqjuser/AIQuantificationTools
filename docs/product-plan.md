...
- 阶段 5 · 实盘准备：当前唯一交付阶段；正在推进本地 fake adapter 的 Shadow Safety Drills，不连接真实券商或资金账户。
...
阶段 4 只允许维护和阻断性缺陷修复；Stage 5 当前只允许按已确认的 shadow safety 范围推进，真实 broker、账户同步、订单提交和 live route 仍属后续规划。
